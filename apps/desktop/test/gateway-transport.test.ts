	...
	const server = Bun.serve({
		...
		websocket: {
			...
			message(ws: ServerWebSocket<undefined>, raw: string | Buffer) {
				received.push(receivedSchema.parse(JSON.parse(typeof raw === "string" ? raw : raw.toString())));
				const frames = script[turn] ?? [{ type: "message", messageId: `message-${turn}`, text: "" }];
				turn += 1;
...
class ControlledSocket {
	...
	addEventListener(type: "message", listener: (event: { data: string | ArrayBuffer | Blob }) => void): void;
...
describe("createGatewayChatTransport", () => {
	...
	test("reply ids stay with their chat when two turns share the socket", async () => {
		...
		const [firstChunks, secondChunks] = await Promise.all([first.then(collect), second.then(collect)]);
		expect(firstChunks.filter((chunk) => chunk.type === "text-delta").map((chunk) => chunk.delta)).toEqual(["answer A"]);
		expect(secondChunks.filter((chunk) => chunk.type === "text-delta").map((chunk) => chunk.delta)).toEqual(["answer B"]);
	...
	test("preserves subprotocol offers on the real socket", async () => {
		...
		const transport = createGatewayChatTransport({ url, protocols: ["openomni", "bearer.test-token"] });
	...
	test("ignores malformed frames and retains unsolicited reply correlation", async () => {
		...
		expect((await collected).filter((chunk) => chunk.type === "text-delta").map((chunk) => chunk.delta)).toEqual(["first"]);

		controlled.receive(JSON.stringify({ type: "message", messageId: "unsolicited", text: "next question" }));
		const reply = collect(await send(transport, [userMessage("answer")]));
		controlled.respond("done");
		await reply;
		expect(controlled.sent).toEqual([{ text: "first" }, { text: "answer", replyToId: "unsolicited" }]);
	});
