...
import { AlarmSourceError, assertAlarmRuntime, commandSource, pathSource, type AlarmSource } from "./alarm-sources";

type Failure = LedgerError | SessionError | AlarmSourceError;
interface Running { readonly row: Alarm.Row; readonly source: AlarmSource }
...
export function createAlarmWorker(options: {
  ...
  return Effect.gen(function* () {
    ...
    const report = (work: Effect.Effect<void, Failure>) => work.pipe(Effect.catchAll((error) => Effect.sync(() => options.failure(error))));
    const offer = (work: Effect.Effect<void, Failure>) => { if (!stopped) queue.unsafeOffer(work); };
    const consumer = yield* Effect.forkIn(Effect.forever(Queue.take(queue).pipe(Effect.flatMap(report))), scope);
    ...
    function release(row: Alarm.Row): Effect.Effect<void, AlarmSourceError> {
      return Effect.suspend(() => {
        ...
        return Effect.tryPromise({ try: () => current.source.close(), catch: () => new AlarmSourceError("source.close") });
      });
    }
    function wake(id: string) {
      return Effect.forkIn(report(options.wake(id)), scope).pipe(Effect.asVoid);
    }
    function deliver(row: Alarm.Row, sourceKey: string, content: string, terminal: boolean, batchHash?: string): Effect.Effect<void, Failure> {
      return Effect.gen(function* () {
        if (stopped) return;
        const fired = yield* options.alarms.fire({
          id: row.id, epoch: row.epoch, fence: row.fence, sourceKey, at: now(), content, terminal,
          ...(batchHash === undefined ? {} : { batchHash }),
        }).pipe(Effect.catchTag("AlarmRefused", (error) => error.reason === "prompt" ? Effect.fail(error) : Effect.succeed(undefined)));
        if (fired === undefined) return;
        if (fired.row.status !== "armed") yield* release(row);
        yield* wake(row.sessionId);
      });
    }
    function summary(row: Alarm.Row, reason: "exit" | "timeout" | "restart" | "source_error", exitCode: number | null) {
      return deliver(row, `${reason}:${row.fence}`, JSON.stringify({ alarmId: row.id, epoch: row.epoch, reason, exitCode }), true);
    }
    function sourceFailure(row: Alarm.Row, error: Error) {
      offer(summary(row, "source_error", null).pipe(Effect.ensuring(release(row).pipe(Effect.orDie)), Effect.ensuring(Effect.sync(() => options.failure(error)))));
    }
    function startCommandWatch(owned: Alarm.Row, watch: Extract<Alarm.Watch, { command: string }>): AlarmSource {
      const filter = watch.filter === undefined ? undefined : new RegExp(watch.filter);
      let lines = 0;
      return commandSource(watch.command, (content) => {
        lines += 1;
        if (filter === undefined || filter.test(content))
          offer(deliver(owned, `line:${owned.fence}:${lines}`, content, false, canonicalDigest(content)));
      }, (code) => offer(summary(owned, "exit", code)), (error) => sourceFailure(owned, error));
    }
    function startWatch(owned: Alarm.Row, preAcquireFence: number): Effect.Effect<void, Failure> {
      return Effect.gen(function* () {
        ...
        const source = yield* Effect.try({
          try: () => "command" in watch ? startCommandWatch(owned, watch) : pathSource(watch,
        ...
        }).pipe(Effect.tapError((error) => summary(owned, "source_error", null).pipe(Effect.ensuring(Effect.sync(() => options.failure(error))))));
    ...
    function start(row: Alarm.Row): Effect.Effect<void, Failure> {
      return Effect.gen(function* () {
        ...
        if (row.kind === "at" && deadline.success) return yield* options.requestTimeout(deadline.data.requestId, now());
        if (row.kind === "at" && Alarm.RetrySchedule.safeParse(row.spec?.value).success)
          return yield* consumeRetry(row);
        const owned = yield* options.alarms.acquire(row.id, row.fence).pipe(Effect.catchTag("AlarmRefused", () => Effect.succeed(undefined)));
    ...
    function observe(entry: Running): Effect.Effect<void, Failure> {
      return Effect.gen(function* () {
        ...
        if (current?.status !== "armed" || current.fence !== entry.row.fence) return yield* release(entry.row);
        const { watch } = Alarm.WatchSpec.parse(current.spec?.value);
        if (watch.timeout_ms !== undefined && now() >= current.fireAt + watch.timeout_ms) yield* summary(current, "timeout", null);
        else entry.source.observe?.();
      });
    }
    const serial = yield* Effect.makeSemaphore(1);
    const tick = () => serial.withPermits(1)(Effect.gen(function* () {
      if (stopped) return;
    ...
    const close = () => Effect.gen(function* () {
      ...
      for (const [id, entry] of running) {
        yield* options.alarms.acquire(id, entry.row.fence).pipe(Effect.catchTag("AlarmRefused", () => Effect.void));
    ...
    return {
      ...
      start: () => Effect.gen(function* () {
        ...
        unsubscribe = options.observations.subscribe(L0Observation.ActionCommittedEvent, (payload) => {
          if (payload.kind === "alarm.arm") offer(tick());
        });
        yield* tick();
        cancelTick = (options.schedule ?? ((callback) => {
