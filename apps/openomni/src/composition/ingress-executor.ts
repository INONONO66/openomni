...
export function createIngressExecutor(clock: () => number): Run {
	...
	SessionHandleStore.materialize({
		...
		actionId: crypto.randomUUID(), at: clock(),
	});
	...
	return (_sender, request, body) => {
		const operation = tail.then(async () => {
			...
			const lease = SessionHandleStore.acquireLease({
				sessionId: id, owner, expectedFence: row.leaseFence,
				now: clock(), expiresAt: clock() + SessionHandleStore.LEASE_TTL_MS,
			});
				...
				ledger: {
					async commit(action) {
						...
						const committed = SessionHandleStore.commit({
							sessionId: id, owner, fence: lease.fence, now: clock(),
							expectedRevision: current.revision, actions: [action], consumeInboxIds: [],
							state: "idle", releaseLease: false,
						});
			...
			try {
				...
				SessionHandleStore.commit({
					sessionId: id, owner, fence: lease.fence, now: clock(),
					expectedRevision: current.revision, actions: [], consumeInboxIds: [],
					state: "idle", releaseLease: true,
				});
			}
		});
