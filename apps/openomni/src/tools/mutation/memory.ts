...
const Output = z.discriminatedUnion("action", [
  z.object({ action: z.literal("add"), store: z.enum(MEMORY_STORES), id: z.string().min(1) }).strict(),
  z.object({ action: z.literal("replace"), store: z.enum(MEMORY_STORES), id: z.string().min(1) }).strict(),
  z.object({ action: z.literal("remove"), store: z.enum(MEMORY_STORES), id: z.string().min(1) }).strict(),
]);

export const MEMORY_TOOL_NAME = "memory";

function executor(memory: CuratedMemory) {
  return async ({ action, store, id, content }: z.output<typeof Input>): Promise<z.output<typeof Output>> => {
    try {
      switch (action) {
        case "add":
...
export const memoryTool = defineTool({
  ...
  bind: (ports) => ports.memory === undefined ? undefined : executor(ports.memory),
  render: (_args, value) => {
    switch (value.action) {
      case "add":
