...
test("bus scan exceptions are reported without escaping the publication", () => Storage.withIsolation(async () => {
  const reported = Promise.withResolvers<Error>();
  const fixture = alarmFixture(":memory:", reported.resolve);
  fixture.worker.start();
  const due = spyOn(fixture.storage.alarms, "due").mockImplementation(() => { throw new Error("SCAN_FAULT"); });
  try {
    fixture.events.publish(L0Observation.ActionCommittedEvent, {
      id: "scan", sessionId: "monitor-session", revision: 1, kind: "alarm.arm",
    });
...
test("the default periodic scan runs at one second and contains timer failures", () => Storage.withIsolation(async () => {
...
test("a physical source close rejection is tracked and reported during shutdown", () => Storage.withIsolation(async () => {
  const fixture = alarmFixture();
  const first = fixture.next("close-fault");
  fixture.arm("close-fault", { command: "printf 'READY\\n'; read hold", description: "close fault", persistent: true });
  fixture.worker.start();
  await first;
  const fault = new Error("CLOSE_FAULT");
  const original = Bun.Terminal.prototype.close;
  let failed = false;
  const close = spyOn(Bun.Terminal.prototype, "close").mockImplementation(function (this: Bun.Terminal) {
    original.call(this);
    if (!failed) {
      failed = true;
      throw fault;
