...
test("a cell batches delegation into one turn", async () => {
  ...
  expect(answer).toContain("offered=[await_delegation,cancel_delegation,complete_work,converse_close,converse_open,delegate,llm,machines,memory,read_artifact,run_code,work_items,write_artifact]");
...
test("the machine tool is not offered while nothing is attached", async () => {
  ...
  expect(offered).toEqual(["delegate", "await_delegation", "cancel_delegation", "converse_open", "converse_close", "machines", "memory", "work_items", "complete_work", "llm", "write_artifact", "read_artifact"]);
