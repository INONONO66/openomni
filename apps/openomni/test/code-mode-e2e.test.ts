...
const sandboxWorkspace = mkdtempSync(join(tmpdir(), "openomni-app-sandbox-tests-"));
 ...
 */
sandboxTest(
  "a cell batches delegation into one turn",
  async () => {
    ...
    expect(answer).toContain(`machines=${MACHINE_ID} — attached, may: kernel.py, sandbox.process`);
    // The cell door offers the same discovery tool — production wiring, not a test-built catalog.
    expect(answer).toContain(`| body: ${MACHINE_ID} — attached, may: kernel.py, sandbox.process`);
  },
  60_000,
);

test.skipIf(process.platform === "linux")(
  "run_code is absent when the attached daemon cannot provide process isolation",
  async () => {
    // Given: an enrolled daemon asks for code execution on an unsupported host.
    let offered: string[] = [];
    const socketPath = testSocketPath();
    const app = await suite.boot({
      ...
      llm: {
        ...
        run: async (input: RunInput, sink: Sink) => {
          ...
          const listed = await input.toolExecutor?.({
            id: "call-2",
            tool: "machines",
            input: {},
          });
    ...
    // When: the actual Resident tool surface is assembled for that attachment.
    const ws = await openSocket(`ws://127.0.0.1:${app.port}/ws?token=${WS_TOKEN}`);
    ...
    // Then: run_code is absent, forced execution is refused, and no unsafe fallback runs.
    expect(offered).not.toContain("run_code");
    expect(answer).toContain(
      'tool "run_code" requires capabilities no attached target holds: kernel.py, sandbox.process',
    );
    expect(answer).toContain(`machines=${MACHINE_ID} — attached`);
  },
  30_000,
);
 ...
 */
sandboxTest(
  "a cell cannot present another cell's id when calling back",
  async () => {
    const socketPath = testSocketPath();
    const served: string[] = [];
...
sandboxTest(
  "cells from different sessions never share interpreter state",
  async () => {
...
sandboxTest(
  "a cell's llm(prompt) is answered by the LlmPort wired into the catalog",
  async () => {
...
sandboxTest(
  "a failing llm call raises ToolError inside the cell instead of returning failure text",
  async () => {
    const { host, run } = await startCellHarness({
      llm: async () => {
...
sandboxTest(
  "parallel() runs independent tool calls concurrently and returns both results in input order",
  async () => {
    let inFlight = 0;
    let maxInFlight = 0;
...
sandboxTest(
  "write_artifact stores from inside a cell and read_artifact fetches it back by id",
  async () => {
