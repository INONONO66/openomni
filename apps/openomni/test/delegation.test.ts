...
describe("delegation controls and tool surface", () => {
  test("start advertises v2 operation and durable controls return immediately", async () => {
    ...
    // #807 — the declaration must be reachable from the advertised surface,
    // or the model can never produce anything but an unverified assign.
    expect(Object.keys(schema.properties)).toContain("verification");
