...
export function residentRunner(options: Omit<ResidentOptions, "sessionRuntime"> & { sessionRuntime?: SessionRuntime }) {
	...
	return {
		...
		async prompt(sessionId: string, content: string) {
			...
			commitMessageInbox({
				id: crypto.randomUUID(), sessionId, kind: "prompt", content,
				origin: { encodingVersion: 1, value: { kind: "test" } },
				createdAt: (runtime.clock ?? Date.now)(), parentActionId: null,
				...(exists ? {} : { createSession: resident.materialize(sessionId, null, "resident", "resident") }),
			});
			const result = await wakeSession(sessionId, resident.runnerFor(SessionHandleStore.row(sessionId)), runtime);
