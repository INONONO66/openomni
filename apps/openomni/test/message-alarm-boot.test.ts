...
test("the shipped startup alarm owner fires exactly at the deadline and never twice after restart", async () => {
  ...
  let calls = 0;
  const options = {
    config, sessionRuntime: { clock: () => now },
    llm: { resolveModel: fakeProviderModel, run: async (input: RunInput, sink: Sink) => {
      calls += 1;
  ...
  const unsubscribe = Bus.subscribe(L0Observation.ActionCommittedEvent, (event) => {
    ...
    if (action !== undefined && SessionHandleStore.turnTerminal(action) !== undefined) terminal.resolve();
  });
  let id: string;
  try {
    const receipt = await app.gateway.ingest({ kind: "external", surface: "ws", externalId: "owner" }, {
  ...
  expect(Storage.get().actions?.append({
    id: "alarm-source", sessionId: id, parentId: null, kind: "message",
    intent: { encodingVersion: 1, value: { messageId: "deadline-request" } },
    effect: { encodingVersion: 1, value: { state: "open" } }, irreversible: true, ts: now,
  }, row.revision)).toBeDefined();
  SessionHandleStore.armMessageDeadline({ messageId: "deadline-request", sessionId: id, sourceActionId: "alarm-source", fireAt: 100, createdAt: now });
  Storage.reset();
  calls = 0;
  now = 99;
  const early = await suite.boot(options);
  expect(SessionHandleStore.inboxRows(id).filter((item) => item.id === "alarm-source:timeout")).toEqual([]);
  expect(calls).toBe(0);
  await early.stop();
  now = 100;
  ...
  const stop = Bus.subscribe(Gateway.MessageObserved, (observation) => {
    if (observation.kind === "message.timed_out") {
      expect(SessionHandleStore.inboxRows(id).some((item) => item.id === "alarm-source:timeout")).toBe(true);
  ...
  expect(observations[0]).toMatchObject({ kind: "message.timed_out", messageId: "deadline-request", waitedMs: 90, sessionId: id });
  expect(SessionHandleStore.inboxRows(id).filter((item) => item.id === "alarm-source:timeout")).toHaveLength(1);
  await due.stop();
