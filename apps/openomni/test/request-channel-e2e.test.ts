...
test("real external WebSocket reply wakes its original idle request owner without legacy writes", async () => {
  ...
  suite.defer(Bus.subscribe(L0Observation.ActionCommittedEvent, (event) => {
    if (event.kind !== "turn") return;
    const terminal = SessionHandleStore.turnTerminal(SessionHandleStore.tree(event.sessionId).find((action) => action.id === event.id));
  ...
  const config = suite.config("request-channel-", {
    ...
    socialBudgets: [{ id: "peer-budget", targetActorId: "peer", maxPerWindow: 5, windowMs: 1000, cooldownMs: 0 }],
  });
  const app = await suite.boot({
    config, sessionRuntime: { clock: () => 100 },
    llm: { resolveModel: fakeProviderModel, run: async (input, sink) => {
      if (!sent) {
        const result = requestToolStep(input, sink, { id: "external-request", tool: "sendMessage", input: {
          to: { kind: "actor", actorId: "peer" }, type: "message", content: "QUESTION_SENTINEL", deadline: 1000,
        } });
      ...
      sink.onMessage(assistantMessage(input, { text: hasAnswer ? "DONE_EXTERNAL_SENTINEL" : "WAITING_EXTERNAL_SENTINEL" }));
      return { type: "stop" };
    } },
  });
  const owner = await suite.openSocket(`ws://127.0.0.1:${app.port}/ws?actor=owner`, ["auth", "token"]);
  const peer = await suite.openSocket(`ws://127.0.0.1:${app.port}/ws?actor=peer`, ["auth", "token"]);
  ...
  peer.send(JSON.stringify({ text: "EXTERNAL_ANSWER_SENTINEL", replyToId: delivery.messageId, eventId: "peer-answer" }));
  ...
  expect(SessionHandleStore.inboxRows(source).filter((row) => row.id === request?.replies[0]?.replyId)).toHaveLength(1);
