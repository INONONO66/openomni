...
| E8 / A15 truncation sub-condition | CLOSED: central 32,000-character model cap includes literal truncated, dropped count and original count; cell output remains full and no spill port is required. | `packages/agent/test/tool-dispatcher.test.ts`; local/real-daemon large-file tests |
