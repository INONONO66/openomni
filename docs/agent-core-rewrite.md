...
| [#645](https://github.com/INONONO66/openomni/pull/645) | deterministic no-LLM reduction: `compaction/reduce.ts` elides old completed tool outputs (sized marker + head excerpt, part identity preserved, idempotent) **before** the lossy cut — the cut becomes the fallback for a history with nothing left to elide. Opt-in strategy block (`elideToolOutputs: {minOutputChars, keepHeadChars}`) through the plan schema. Yield reporting feeds the later adaptive row via the next measured call | 🟨 |
| — | cut planning, incremental summarization | ⬜ |
