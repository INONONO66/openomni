...
| [#644](https://github.com/INONONO66/openomni/pull/644) | `compaction/measure.ts` — the trigger reads the provider-measured context of the last call (`input + cache.read + cache.write`, recorded off the tracking sink where tokens land, exposed as `PolicyContext.contextTokens`). The old trigger summed cumulative run spend, which re-counts every prior turn's input and only ever grows — it would fire on long runs whose window was nowhere near full. An unmeasured seam skips and records the skip | 🟨 |
