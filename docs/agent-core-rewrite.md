...
**We are already on the pss side of that split and should stay there.** For us the harness content is product and lives in the app and ledger.
...
Consequence, now realised: `llm` depends on protocol and telemetry only, and `agent` carries no dependency on durable storage. `script/check-deps.ts` no longer lists `@openomni/ledger` in either allowlist and neither manifest declares it, so the boundary is enforced rather than conventional — reopening either edge fails the gate. Relocating `Bus` made it possible; converting the nine imports is what achieved it.
...
| [#616](https://github.com/INONONO66/openomni/pull/616) | `agent` and `llm` drop `@openomni/ledger`; the allowlists close behind them | ✅ |
...
| [#612](https://github.com/INONONO66/openomni/pull/612) | move `TraceContext` off `packages/ledger` — it owned a second, contradictory trace convention | ✅ |
...
| [#642](https://github.com/INONONO66/openomni/pull/642)+[#643](https://github.com/INONONO66/openomni/pull/643) | compaction registration (config parse + `registry.register` + the `priority: 900` opinion) moved to openomni's `compaction-policy.ts`; the agent-owned built-in registry was deleted. #643 temporarily pinned the generic registry's resolve contract; the later dead-surface audit removed that unused abstraction and its tests entirely | ✅ |
...
| — | cut planning, incremental summarization — resolved by design into anchored iterative summarization with user-verbatim preservation; target and leaf map live in [`docs/compaction-design.md`](compaction-design.md) (L2) | ⬜ |
| — | speculative overlap and bounded overflow retry (D8) — resolved by design into prepare/promote speculation with a freshness guard plus provider-overflow recovery; target and leaf map live in [`docs/compaction-design.md`](compaction-design.md) (L4, L5) | ⬜ |
...
| — | `WorkerBootstrap.ToolCatalogEntry.mcpServer` (pre-#500 name: `RuntimeToolCatalogEntry`) is write-only on the wire — no production reader (#640 review); grow a consumer or delete the field (protocol schema surface, Owner review) | ⬜ |
...
The removed product Stakes implementation was 1,033 LOC of mechanism whose
...
3. ~~Core may not import `builtin/`.~~ **Resolved by dissolution, not by a
   ...
   config from the product. The temporary replacement lock became vacuous when
   the unused generic registry abstraction was deleted; the agent package now
   exposes only the engine mechanism and caller-supplied registrations.
