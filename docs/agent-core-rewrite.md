...
| — | cut planning, incremental summarization — **deferred pending the wiring ruling below**: further layers on a dormant feature are speculative until the product supplies compaction config | ⬜ |
| — | speculative overlap and bounded overflow retry (D8) — same deferral | ⬜ |
| — | **Owner ruling requested: wire compaction live.** The machinery is now measured (#644), reduction-first and convergent (#645), and registered through the plan path (#642) — but no production caller supplies `WorkerMiddlewareConfig.compaction` or a plan config, so none of it runs. Turning it on is a product decision (which window/threshold per model, whether to enable elision, whether a summarizer is wired), and the remaining Phase 3 rows (adaptive yield feedback, speculative overlap) only earn their complexity against live usage | ⬜ |
