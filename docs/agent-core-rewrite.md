...
| [#649](https://github.com/INONONO66/openomni/pull/649) | **Compaction wired live (Owner ruled: proceed, 2026-08-16).** The window is the loop's fact — `run.ts` records the resolved model's `limit.context`, the dispatch context exposes it, and strategy config may only narrow it (`contextWindowTokens` now optional everywhere). `buildWorkerMiddleware` registers `builtin:compaction` by default with the product strategy: elision knobs only (min 4000 chars, head 500), no summarizer — the boundary-snap cut is deterministic. Unknown window (proxy models report 0) skips with `compaction_skipped_no_window`, recorded | 🟨 |
