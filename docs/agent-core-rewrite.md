...
| — | D11 remainder — 17 mint sites: coordinator (7 — six supervisor generation traces + the socket-dir sweep, all **Owner-gated** ring-2 dep set), `cron-job-registry.ts`'s register/cancel pair (behind the dispatch `Scheduler` owner interface), and apps/server's discord `onReady`, `resolveDefaultProviderModel`, fetch-retry, channel-authn, context loaders (8). Beyond the mint count, two structural leftovers from the #655 review: **~29 event definitions across whole families (Wait, PendingInteraction, PendingAsk, WorkerGrant, Session, messaging, injection-queue) carry no `traceId` field at all** — every production row from them persists under the `"untraced"` sentinel, so the sentinel is steady-state until those schemas gain the field (additive, per-family batches); and the HTTP request middleware's `requestId` mint is still a dashed uuid that now reaches `trace_id` through the reconciler — converting it to W3C changes the externally visible `X-Request-Id` format, so it stays until that trade is ruled | ⬜ |
