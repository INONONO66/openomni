...
| — | D11 remainder — 17 sites: coordinator supervisor generation traces (7, **Owner-gated** ring-2 dep set), `cron-job-registry.ts`'s register/cancel pair (behind the dispatch `Scheduler` owner interface), and apps/server's discord `onReady`, `resolveDefaultProviderModel`, fetch-retry, channel-authn, context loaders (8) | ⬜ |
