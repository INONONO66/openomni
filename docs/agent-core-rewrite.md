...
| D11 | Opaque trace ids convert **at the origin**, never in the emitter. No normalizing adapter exists, and a test pins its absence | `requireTraceScope` takes W3C 32-hex; 115 sites across 44 files still pass a dashed `crypto.randomUUID()` as `traceId`. Those are per-line ids wearing a trace's name — each one correlates to exactly one row. A normalizer would make them *look* correlated while leaving every caller minting its own vocabulary, so the shortcut is refused: Phase 1b converts each site to inherit a scope, or to mint via `newTraceId()` where it is a genuine origin. Until then `scope()` is on no production path. |
