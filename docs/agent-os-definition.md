...
OpenOmni sharpens that duty because every unit of work is a WorkItem contract. Progress completion, one execution's exit status, one criterion's result, and terminal contract completion are different facts. The shipped #490 completion authority closes a WorkItem only after a durable admission bound to the current contract revision and evidence basis; internal and connector Workers are the wired consumers. The exhaustive origin projector is the required seam for future external, human, SDK/internal, replay, recovery, and Resident integrations, not evidence that those caller paths are already wired.
