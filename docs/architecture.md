...
**Target policy topology:** policy is a system-wide, actor-agnostic interception plane rather than a Worker subsystem. Resident, Worker, Jester, Governor, ingress, dispatch, memory, scheduling, tools, LLM connections, and writeback may receive registrations selected by actor profile and context. **Current wiring:** protocol provides 18 contract-grade points (`dispatch.action.pre`, `run.lifecycle/turn/completion/error.*`, `work.complete.pre`, `prompt.context.pre`, `connection.llm.pre/post`, `tool.catalog/native/mcp.*`, `delegation.worker.pre/post`); the shared agent loop, WorkItem completion authority, Worker path, dispatch, injection queue, server context middleware, and MCP guard dispatch their applicable points. Generic agent-loop `run.completion.pre` and WorkItem contract-closing `work.complete.pre` are intentionally distinct. Resident-specific profile selection, Jester/Governor attachment, and memory/schedule points remain target work called out below.

Current status and work remaining:

1. **Three remaining points + ID-grammar extension** (`memory|egress|schedule` prefixes): `memory.recall.pre` (scope filter), `egress.render.pre` (voice contract), and `schedule.fire.pre` (cron constraints). `work.complete.pre` is shipped, fail-closed, and alias-free. Note: the grammar's `credential` prefix has zero registered points — consistent with the coordinator credentials/tool-permission code being dead (deleted in #461).
...
ring 5  apps/server             composition root (registers channel drivers, composes the gateway router); zero direct ledger imports
```
...
Approved target architecture (recorded 2026-08-03; the normative decision text lives in the #459 leaf contracts — receipt v6 — and passages marked *illustration* below are non-normative sketches, not recorded contract text). None of this is current wiring; [Implementation Status](implementation-status.md) stays the shipped-state source of truth.
...
**Outbound target selection is `@openomni/placement`** (pick one executor among capability-tagged candidates), a ring-1 pure decision package beside policy (opened at #752 with the model-fallback axis — `Placement.selectModel`, consumed by the agent loop's per-attempt resolution; the executor/capability axes remain target). It maps `(command, capability tags, declared facts, policy constraints) → target decision` with declared fallback chains (the implemented reason classes `timeout`/`transient_error`/`validation_error` → next candidate; broader failure labels normalize to these before selection). It decides placement only: policy alone owns allow/deny, admission alone closes work, and the selection result is consumed as an input, like Stakes. Inbound routing (`resolveRoute`) is a perimeter gate concern owned by the `@openomni/channels` gateway (superseded 2026-08-19, [gateway-design.md](gateway-design.md) §8.4; landed at gateway stage 2, #736 — the external arms live in `packages/channels/src/router/`, the internal arm stays kernel-side). A network path (VPN, tailnet, proxy exit) is a capability tag and endpoint property on the target — never a kernel concern (*illustration*: per-command network wrapping as a `remote`/`browser` execution-profile option); how the effective path is recorded belongs to the #510 attempt-identity and #492 effect contracts — this document does not extend their enumerations.
