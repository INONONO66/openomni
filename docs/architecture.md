...
3. Add canonical `dispatchPoint` execution that parses each point's input schema, enforces `requiredContext`, and rejects effects outside the registration's declared capability subset; then remove fabricated context from `DispatchRuntime`.
4. Migrate production policy call sites to canonical point dispatch, including `delegation.worker.pre/post`. The legacy timing API remains isolated during migration.
...
| **P1 one channel — partial** | 🚧 The coordinator/worker-driver slice shipped in #477-#481: injected ports and a session-free coordinator, typed `deliver`, gate-side policy-plan stamping, one pool with supervisor options, and driver lifecycle/wall-time enforcement. The policy-contract foundation now provides executable schemas for all 19 points and registration-time `allowedEffects` validation. Remaining: enforce schemas, required context, and declared effect subsets in canonical `dispatchPoint`; remove fabricated `DispatchRuntime` context; migrate production policies and gate delegation (#454); land #464's single `resolveRoute` pipeline with exactly one `RoutingDecision` ledger event per inbound message; close server back doors; delete ingress's double implementation; collapse to one tool-executor path |
