...
| L7 | Byte guard enforced at the wrapper (multiset byte-identity of user text, violation refuses the effect visibly) + deterministic seeded probe bench (Factory's probe methodology as byte-presence checks with a tokens-per-task floor; blind LLM judge stays pluggable/manual) including the user-verbatim vs uniform A/B | openomni (guard + bench) | L2–L6 |
