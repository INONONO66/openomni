...
- Zone layout `[toggle] [spacer] [trio]`, `gap-1`, `pr-3`: the trio (`TabStrip.Trio`, `ml-auto`) is RIGHT-ALIGNED to the sidebar's edge, 12px in, and rides `--sidebar-width` while open; collapsed, the zone's width equals its content so the spacer is zero and the trio sits 4px after the toggle. The trio is ONE node for the window's life (never re-keyed; `test/strip-mount.test.tsx` renders it on a live DOM, collapses and re-opens, and asserts the same element with one mount): it carries `data-collapsed`, and `strip-trio` (`styles.css`) registers `--strip-trio-progress` (`@property`, 0 open / 1 collapsed) that TRANSITIONS on `--duration-base` / `--ease-frame` — the zone's own width motion — with opacity `clamp(0, 1 - 3 * min(p, 1 - p), 1)`: the reference's `clamp(1 - 3p, 0, 1)` with `p` read from the nearer end, so the trio is out for the first third of the travel and back over the last. Nothing re-renders for the fade; the attribute flips and CSS does the rest.
