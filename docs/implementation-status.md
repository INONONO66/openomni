...
| Completion-report evidence gate (claims → evidence refs; unevidenced = not done) | ✅ | `packages/session/src/work-item/`, `packages/openomni/src/dispatch/handlers/worker.ts` | Deterministic gate: `WorkItemStore.complete()` requires a completion report whose claim evidence IDs resolve to ledger evidence; `worker.spawn` succeeded results without a valid evidence-backed report remain blocked instead of completed. Read-back verification remains pending below |
