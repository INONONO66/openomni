...
| `PendingInteraction` (successor; correlation routing, follow-up window) | 🚧 | `packages/protocol/src/communication/`, `packages/session/src/pending-interaction/`, `packages/session/src/storage/sqlite-pending-interaction-adapter.ts`, `packages/session/migration/0009_pending_interaction/migration.sql` | Canonical PI schema and SQLite-backed store exist with `open / resolved / follow_up / expired / cancelled`, `allowedActions`, `workerRunId`, correlation lookup, and follow-up window filtering. No PI-match session override exists in dispatch yet |
...
| Boot-time PendingInteraction restoration | 📋 | — | Depends on PI dispatch wake-up wiring |
