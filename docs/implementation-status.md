...
| `PendingInteraction` (successor; correlation routing, follow-up window) | 🚧 | `packages/protocol/src/communication/`, `packages/protocol/src/dispatch/`, `packages/session/src/pending-interaction/`, `packages/session/src/storage/sqlite-pending-interaction-adapter.ts`, `packages/session/migration/0009_pending_interaction/migration.sql`, `packages/openomni/src/dispatch/pending-interaction-routing.ts` | Canonical PI schema and SQLite-backed store exist with `open / resolved / follow_up / expired / cancelled`, `allowedActions`, `workerRunId`, correlation lookup, and follow-up window filtering. Dispatch can pre-authorize route structured `actor.message` correlation matches to `actor.reply`, override the target session/run to the owning WorkerRun, and record `pending_interaction_scope` as the effectiveAuthority allow axis. Server channel adapters still do not submit inbound messages through this PI route |
...
| Boot-time PendingInteraction restoration | 📋 | — | Depends on server/channel wake-up wiring |
