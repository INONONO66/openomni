...
- #949 is complete: stage 3 landed `eval.op = run | peek | stop` over a background cell registry, `completion` options, and tool-named codemode handle methods (`read/write/ls/bash/eval`). Continuous alarm scheduling stays #947; #969 acceptance uses the behavioral and deletion receipts above, not only its census; machine handles and codemode are described above.
