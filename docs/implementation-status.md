...
| `local_cli_agent` executor (CLI agents as installed apps) | 🚧 | `packages/protocol/src/dispatch/index.ts`, `packages/openomni/src/execution-runtime/tool/agent/tools/dispatch.ts`, `packages/openomni/src/dispatch/handlers/worker.ts`, `packages/openomni/src/dispatch/handlers/worker-work-item.ts`, `packages/openomni/src/app-connector/` | Dispatch targets can now carry `executorKind`, the model-facing dispatch tool exposes the canonical executor-kind enum, `worker.spawn` fail-closes unsupported non-internal executors with WorkItem ledger evidence instead of silently routing them through the internal ChatAgent, OpenOmni publishes first-party declarative connector definitions for Claude Code, Codex CLI, and OpenCode, and the built-in discovery API evaluates local detect commands into install candidates. Actual registration/consent/wiring/smoke verification and local CLI runtime/process spawning remain pending per ADR-010 §3 |
| `AppConnector` schema (declarative connector definition = public ABI) | ✅ | `packages/protocol/src/app-connector/`, `packages/openomni/src/app-connector/` | ADR-010 §3 schema shipped: detect/spawn/logs/question bridge/evidence/requires/profile; built-in first-party definitions parse through the public ABI, and discovery consumes the public detect contract. Install lifecycle registration and runtime wiring remain pending below |
| Install lifecycle (discover → register → consent → wire → smoke-verify; version-drift re-verification) | 🚧 | `packages/openomni/src/app-connector/discovery.ts` | Built-in connector discovery now runs each connector's detect command, extracts versions from the connector `versionPattern`, classifies missing / unsupported / failed / available candidates, and treats tested version ranges as the first drift guard. Register, consent, hook/credential wiring, smoke verification, and drift incident handling remain pending |
