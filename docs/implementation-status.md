...
**Legend**: ✅ implemented and wired · 🔌 dormant — built and tested, zero production callers · 🚧 partial · 📋 designed, not implemented · Last verified: **2026-06-13** (update this date when re-auditing).
...
| `executorKind` on WorkerRun (`internal_chat_agent` / `local_cli_agent` / `external_api` / `a2a` / `human_channel`) | ✅ | `packages/protocol/src/subagent/worker-run.ts`, `packages/session/src/worker-run/`, `packages/session/migration/0005_worker_run_executor_kind/migration.sql` | WorkerRun records now carry durable `executorKind`; `WorkerRun.create()` defaults current in-process Worker runs to `internal_chat_agent`. Non-internal executor runtimes remain pending in the connector / outbound-action rows below |
| `local_cli_agent` executor (CLI agents as installed apps) | 🚧 | `packages/protocol/src/dispatch/index.ts`, `packages/openomni/src/execution-runtime/tool/agent/tools/dispatch.ts`, `packages/openomni/src/dispatch/handlers/worker.ts`, `packages/openomni/src/dispatch/handlers/worker-work-item.ts`, `packages/openomni/src/app-connector/` | Dispatch targets can now carry `executorKind`, the model-facing dispatch tool exposes the canonical executor-kind enum, `worker.spawn` fail-closes unsupported non-internal executors with WorkItem ledger evidence instead of silently routing them through the internal ChatAgent, OpenOmni publishes first-party declarative connector definitions for Claude Code, Codex CLI, and OpenCode, the built-in discovery API evaluates local detect commands into install candidates, available candidates can be registered as durable connector installation records with Owner consent metadata, and consented records can be smoke-verified into `enabled` or `verification_failed`. Actual local CLI runtime/process spawning remains pending per ADR-010 §3 |
| `AppConnector` schema (declarative connector definition = public ABI) | ✅ | `packages/protocol/src/app-connector/`, `packages/openomni/src/app-connector/` | ADR-010 §3 schema shipped: detect/spawn/logs/question bridge/evidence/requires/profile plus durable installation and consent records; built-in first-party definitions parse through the public ABI, discovery consumes the public detect contract, and registry stores installation records. Runtime wiring remains pending below |
| Install lifecycle (discover → register → consent → disable/uninstall → wire → smoke-verify; version-drift re-verification) | 🚧 | `packages/openomni/src/app-connector/discovery.ts`, `packages/openomni/src/app-connector/registry.ts`, `packages/session/src/app-connector/` | Built-in connector discovery now runs each connector's detect command, extracts versions from the connector `versionPattern`, classifies missing / unsupported / failed / available candidates, treats tested version ranges as the first drift guard, and registers available candidates in SQLite-backed installation storage through `register()`. Owner consent is created later through `requestConsent()` / `grantConsent()` and persisted as a permission ceiling constrained to connector requirements. The lifecycle SSOT also exposes durable `disable()` and strict `uninstall()` operations for installation records, plus smoke verification that reruns the detect contract through an injectable runner: successful consented installs become `enabled`, and missing / failed / unsupported detects become `verification_failed` while preserving consent. Hook/credential wiring and drift incident handling remain pending |
...
| Connector/channel-specific read-back request builders | 📋 | — | App connector and channel adapters do not yet derive source-specific `readBackRequests` automatically from publish/send/API actions |
...
| Owner adoption signal storage primitive (`outcome`: adopted/corrected/redone/ignored) | ✅ | `packages/protocol/src/work-item/index.ts`, `packages/session/src/work-item/outcome.ts` | `WorkItem.Info.outcome` is schema-backed and `WorkItemStore.recordOutcome()` records Owner feedback on completed WorkItems while publishing `work_item.outcome_recorded` for downstream Governor consumers |
