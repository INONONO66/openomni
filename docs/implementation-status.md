...
| Tool catalog and prompts | Stage 1 adds read/write/edit/list/search/bash alongside the retained delegation, approval, provisioning, cell execution and cell LLM definitions. The prompt builder accepts model tuning only; deleted-domain injection/instructions are absent. Dispatcher-only model truncation caps at 32,000 UTF-16 code units on a Unicode code-point boundary, retaining a literal truncated marker with exact dropped and original UTF-8 byte counts; cell values stay full. No artifact spill occurs. | `apps/openomni/src/tools/core/catalog.ts`, `apps/openomni/src/prompt/`, `packages/agent/src/tool-dispatcher.ts` |
