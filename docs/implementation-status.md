...
| `local_cli_agent` executor (CLI agents as installed apps) | 🚧 | `packages/protocol/src/dispatch/index.ts`, `packages/openomni/src/execution-runtime/tool/agent/tools/dispatch.ts`, `packages/openomni/src/dispatch/handlers/worker.ts`, `packages/openomni/src/dispatch/handlers/worker-work-item.ts` | Dispatch targets can now carry `executorKind`, the model-facing dispatch tool exposes the canonical executor-kind enum, and `worker.spawn` fail-closes unsupported non-internal executors with WorkItem ledger evidence instead of silently routing them through the internal ChatAgent. Actual local CLI connector runtime/process spawning remains pending per ADR-010 §3 |
