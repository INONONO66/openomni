...
| Task ledger view ("show open tasks" — the OS's `ps`) | ✅ | `apps/server/src/handler/conversation.ts` | Chat command returns a capped pending/running/blocked WorkItem diagnostic from `WorkItemStore`; actor-scoped visibility remains future work because current `WorkItem` / `InboundMessage` schemas have no owner, trust-tier, or channel-grant field; web view also remains future work |
