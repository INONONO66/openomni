...
- An executable verifier result satisfies only the criterion its declaration bound it to at delegation time (`command.v1.expectations[].criterionIndex`); `checkedPredicate` records the executable identity, argv digest, and expected observation. Citation support remains the text-match exception because its frozen classifier evaluates the persisted criterion text directly. An unrelated green predicate never closes another criterion.
...
- **Verifiers are deterministic code, sandboxed** (no network, no clock, no undeclared subprocess; deny-by-default) — purity by capability, not by naming convention. `command.v1` may launch only the executable selected from the Owner's registered executable map; verifier input can supply literal argv but cannot select an arbitrary binary. **No LLM-in-verifier.** Four families, strongest first: executable re-check > citation/quote match > frozen-NLI support > constrained-decoding validity (validity only, never promoted to truth). Every verifier's bench must demonstrate discrimination (returns `refuted` on a known-bad input).
