...
| `providerOptions?` | `Record<string, unknown>`              | Forwarded verbatim to the underlying provider SDK. Untyped on purpose (the shape is the provider's, per provider and per SDK version); the HOST owns validating it — neither this loop nor the llm package inspects it |
