...
export function createSessionTurn(
	...
	async function seal(
		...
		if (reply === undefined) committed = SessionHandleStore.commit(commit);
			...
			if (runtime.commitTerminal === undefined) throw new Error("child terminal requires the gateway commit port");
			committed = await runtime.commitTerminal({ commit, reply, policy: pinPolicy(open.policyGeneration) });
		}
		requireCommit(committed);
