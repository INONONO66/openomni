...
const REASONS = ["timeout", "tool_error", "transient_error", "validation_error", "context_overflow"] as const satisfies readonly RetryReason[];
const exhaustive: [Exclude<RetryReason, (typeof REASONS)[number]>] extends [never] ? true : never = true;
void exhaustive;

describe("model selection and retry vocabulary", () => {
  const chain = [{ provider: "test", id: "first" }, { provider: "test", id: "next" }];
