...
describe("tool post-policy refusal", () => {
  it("publishes one completion only after the blocked-post result commits", async () => isolated(Effect.scoped(Effect.gen(function* () {
    ...
    const running = yield* Effect.forkScoped(Effect.runSync(createDispatcher({ executor: recording.executor }).pipe(Effect.provide(catalogLayer([definition])))).execute(call, context));
  ...
  it("returns an error result through the model door", async () => isolated(Effect.scoped(Effect.gen(function* () {
    const result = yield* Effect.runSync(createDispatcher({ executor: recordingExecutor({ policy: accountOutputDeniedPolicy() }).executor }).pipe(Effect.provide(catalogLayer([definition])))).execute(call, context);
    expect(result).toMatchObject({ isError: true, errorKind: "precondition_failed" });
  ...
  it("throws through the cell door", async () => isolated(Effect.scoped(Effect.gen(function* () {
    const exit = yield* Effect.exit(Effect.runSync(createDispatcher({ executor: recordingExecutor({ policy: accountOutputDeniedPolicy() }).executor }).pipe(Effect.provide(catalogLayer([definition])))).executeCell(call, context));
  ...
  for (const door of ["model", "cell"] as const) {
    it(`${door} door uses a valid transformed output`, async () => isolated(Effect.scoped(Effect.gen(function* () {
      const dispatcher = Effect.runSync(createDispatcher({ executor: transformingExecutor("masked") }).pipe(Effect.provide(catalogLayer([definition]))));
    ...
    it(`${door} door rejects a transformed output that breaks the schema`, async () => isolated(Effect.scoped(Effect.gen(function* () {
      const dispatcher = Effect.runSync(createDispatcher({ executor: transformingExecutor(null) }).pipe(Effect.provide(catalogLayer([definition]))));
      const result = yield* (door === "model" ? dispatcher.execute(call, context) : dispatcher.executeCell(call, context));
