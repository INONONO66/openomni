...
import { session, closeSessions, sweepSessions, wakeSession, type SessionRuntime } from "../src/session-handle";
...
test("a dropped receiving consumer leaves a sealed source obligation without mutating its parent", async () => {
  const runtime: SessionRuntime = {
    observations: { publish: () => undefined }, clock: () => 100,
    async dispatchOutbound() { throw new Error("receiver unavailable"); },
  };
  runtimes.push(runtime);
  session({ id: "parent", role: "resident", runner: async () => ({ kind: "result", text: "parent" }) }, runtime);
  const child = session({ id: "child", parentId: "parent", role: "worker", runner: async () => ({ kind: "result", text: "child answer" }) }, runtime);
  const before = SessionHandleStore.tree("parent");
  await expect(child.prompt("work", {
    encodingVersion: 1,
    value: { kind: "message", messageId: "commission", senderSessionId: "parent", sourceActionId: "commission-action" },
  ...
  expect(source.filter((action) => SessionHandleStore.turnTerminal(action) !== undefined)).toHaveLength(1);
...
test("restart after receiving commit retries exact bytes without another inbox or receiver execution", async () => {
  ...
  for (const row of SEEDED_POLICY_ROWS) policies.append({ ...row, generation: 1 });
  let consumed = 0;
  const parentRunner = async () => { consumed += 1; return { kind: "result" as const, text: "received" }; };
  const sent: string[] = [];
  function runtime(at: number, loseAck: boolean): SessionRuntime {
    const value: SessionRuntime = {
      observations: { publish: () => undefined }, clock: () => at,
      async dispatchOutbound({ message }) {
        sent.push(JSON.stringify(message));
        const received = SessionHandleStore.commitReceivedMessage({
          id: message.messageId, sessionId: message.destinationSessionId, kind: "prompt",
          content: message.content, origin: { encodingVersion: 1, value: message },
          createdAt: at, parentActionId: null,
        });
  ...
  actions.append({ id: "commission-action", parentId: null, sessionId: "parent", kind: "message",
    intent: { encodingVersion: 1, value: { phase: "intent", value: { messageId: "commission" } } },
    effect: { encodingVersion: 1, value: { phase: "pending" } }, ts: 100, irreversible: true,
  }, SessionHandleStore.row("parent").revision);
  const child = session({ id: "child", parentId: "parent", role: "worker", runner: async () => ({ kind: "result", text: "exact answer" }) }, first);
  await expect(child.prompt("work", { encodingVersion: 1, value: {
    kind: "message", messageId: "commission", senderSessionId: "parent", sourceActionId: "commission-action",
  ...
  await sweepSessions((row) => row.id === "parent" ? parentRunner : async () => { throw new Error("sealed child replayed"); }, reopened);
