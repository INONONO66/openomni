...
- `script/check-deps.ts` checks manifest and source imports plus S8 banding. Only `src/router/` and `src/authn/` may import policy/ledger; driver files cannot reach router internals. Allowed ledger names are ActorRegistry, BlacklistStore, ChannelGrantStore, ReplyGrantStore, EgressBudgetStore, SurfaceKey, and DecisionFacts. No standalone lifecycle store or master Storage entry is allowed. The agent dependency is test-only.
