...
export class SlackNormalizer implements InboundNormalizer<SlackMessageEvent> {
	...
	normalize(event: SlackMessageEvent, traceId: string): Channel.InboundMessage | null {
		...
		const mentions = [...event.text.matchAll(/<@([^>]+)>/g)].flatMap((match) => match[1] === undefined ? [] : [match[1]]);
		...
		return {
			...
			facts: {
				...
				dm: isDM,
				...(event.thread_ts !== undefined ? { reply: { chain: [event.thread_ts], threadId: event.thread_ts } } : {}),
				payload: event,
				render: event.text,
			},
		};
