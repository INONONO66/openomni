...
export class TelegramNormalizer implements InboundNormalizer<TelegramMessage> {
	...
	normalize(message: TelegramMessage, traceId: string): Channel.InboundMessage | null {
		...
		const addressees = (message.entities ?? []).flatMap((entity) => {
			if (entity.type === "text_mention" && entity.user !== undefined) return [{ externalId: String(entity.user.id) }];
			if (entity.type !== "mention") return [];
			const name = text.slice(entity.offset, entity.offset + entity.length);
			return [{ externalId: name.toLowerCase() === `@${this.ctx.botUsername.toLowerCase()}` ? this.ctx.botId : name }];
		});
		const chain: string[] = [];
		for (let reply = message.reply_to_message; reply !== undefined; reply = reply.reply_to_message) chain.push(String(reply.message_id));
