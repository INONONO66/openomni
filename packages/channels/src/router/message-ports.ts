...
export interface GatewayRouterPorts {
	...
	readonly observe?: (sender: Gateway.IngestSender, observation: Gateway.MessageObservation) => void;
	...
	readonly armDeadline?: (input: {
		messageId: string; sessionId: string; sourceActionId: string;
		fireAt: number; createdAt: number; replyTo?: string;
	}) => void;
