...
/**
 * Effectful gateway Wait service (#215, promoted to the router band at #707
 * stage 2 — the sole wait-store writer). Definitions (schema + fold) live in
 * protocol; durable persistence and fold application live in the ledger
 * WaitStore; this service is the single gateway entry that wires them to
...
export namespace WaitService {
   ...
   * head disagrees with its row) records an Operational.Events.Error and the
   * sweep continues — a single bad row must never kill boot recovery.
   *
   * `publish` is the injected observation sink (channels never imports the
   * observation channel — the llm/agent precedent); the composition root
   * binds it to Bus.publish. `traceId` is the caller's (boot recovery's):
  ...
  export function sweepExpired(
    ...
    for (const record of WaitStore.list(["open"])) {
      ...
      try {
        ...
        publish(Operational.Events.Error, {
          traceId,
          time: Date.now(),
          component: "wait",
