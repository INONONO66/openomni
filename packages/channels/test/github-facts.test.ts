...
for (const text of ["ambient event", "@owner review @bot", "@bot"]) {
  test(`GitHub authenticated webhook preserves platform facts: ${text}`, async () => {
    ...
    try {
      const response = await adapter.handleWebhook(new Request("http://localhost/github/webhook", {
        method: "POST", body, headers: {
          "x-hub-signature-256": `sha256=${createHmac("sha256", "secret").update(body).digest("hex")}`,
          "x-github-event": "issue_comment", "x-github-delivery": "event-42",
        },
      }));
      expect(response.status).toBe(200);
      expect(received).toHaveLength(1);
      expect(received[0]?.sender).toEqual({ kind: "external", surface: "github", externalId: "sender" });
      const facts = Gateway.IngressFacts.parse(received[0]?.facts);
      expect(facts.payload).toEqual(payload);
      expect(facts).toMatchObject({ eventId: "event-42", workspaceId: "owner/repo", channelId: "issue-7", dm: false, render: text });
      expect(facts.addressees).toEqual(text === "ambient event" ? [] : text === "@bot" ? [{ externalId: "bot" }] : [{ externalId: "owner" }, { externalId: "bot" }]);
      expect(facts.reply).toEqual({ chain: [], threadId: "7", externalConversationId: "github:owner/repo:issue:7" });
    } finally { adapter.stop("trace"); }
  });
}
...
test("GitHub refuses malformed destinations and missing credentials before effects", async () => {
  const realFetch = globalThis.fetch;
  let calls = 0;
  globalThis.fetch = Object.assign(async () => { calls += 1; return Response.json({}); }, { preconnect: realFetch.preconnect });
  try {
    const adapter = new GitHubAdapter("secret", {}, () => undefined);
    for (const endpoint of ["owner/repo#7", "owner/repo", "owner/..#7", "owner/repo#999999999999999999999"]) {
