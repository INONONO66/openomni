...
describe("GatewayRouter conversation isolation", () => {
  ...
  test.each(["workspaceId", "channelId"] as const)("different %s isolates the target session", async (field) => {
    const first = await kernelRouter().ingest(ownerSender, ownerFacts);
    const second = await kernelRouter().ingest(ownerSender, { ...ownerFacts, eventId: "second", [field]: "other" });
    if (first.status !== "executed" || second.status !== "executed") throw new Error("not executed");
  ...
  test("allowlist refuses strangers while admitting the authenticated listed sender", async () => {
    ChannelGrantStore.put({ id: "grant", surface: "discord", kind: "trusted_channel", defaultTier: "owner", allowedSenders: [ownerSender.externalId], createdBy: "owner" });
    expect(await kernelRouter().ingest({ ...ownerSender, externalId: "stranger" }, ownerFacts)).toMatchObject({ status: "blocked_pre" });
    expect((await kernelRouter().ingest(ownerSender, { ...ownerFacts, eventId: "allowed" })).status).toBe("executed");
    expect(commits).toHaveLength(1);
  });
  test("invalid facts fail schema validation before routing", async () => {
    await expect(kernelRouter().ingest(ownerSender, { ...ownerFacts, eventId: "" })).rejects.toMatchObject({
