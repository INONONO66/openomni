...
import { commits, kernelRouter, ownerFacts, ownerSender, resetRouterState, routingDecisions } from "./_router-fixture";
...
describe("GatewayRouter access routing", () => {
  ...
  test("default-tier admission never registers a new endpoint", async () => {
    ChannelGrantStore.put({ id: "grant", surface: "discord", kind: "trusted_channel", defaultTier: "owner", createdBy: "owner" });
    const result = await kernelRouter().ingest(ownerSender, ownerFacts);
    expect(result.status).toBe("executed");
    expect(routingDecisions()[0]).toMatchObject({ stage: "surface_default", outcome: "route", trustTier: "owner" });
    expect(commits).toHaveLength(1);
    expect(ActorRegistry.resolveEndpoint(ownerSender.surface, ownerSender.externalId, ownerFacts.workspaceId)).toBeUndefined();
  });
  test.each([undefined, "full_access"] as const)("broadcast treatment %s remains evidence-only", async (inboundTreatment) => {
    ChannelGrantStore.put({
      id: "grant", surface: "discord", kind: "broadcast_channel", defaultTier: "observer", createdBy: "owner",
      ...(inboundTreatment === undefined ? {} : { inboundTreatment }),
    });
    expect((await kernelRouter().ingest(ownerSender, ownerFacts)).status).toBe("executed");
    expect(routingDecisions()[0]).toMatchObject({ outcome: "route", trustTier: "observer", inboundTreatment: "evidence_only" });
    expect(commits).toHaveLength(1);
  ...
  test("blocked channel refuses before inbox commit", async () => {
    ChannelGrantStore.put({ id: "grant", surface: "discord", kind: "blocked_channel", createdBy: "owner" });
    expect(await kernelRouter().ingest(ownerSender, ownerFacts)).toMatchObject({ status: "blocked_pre" });
    expect(routingDecisions()[0]).toMatchObject({ stage: "channel_ceiling", outcome: "block", inboundTreatment: "drop" });
    expect(commits).toEqual([]);
  });
});
