...
test("correlated reply resolves the Wait and commits only to its owner", async () => {
  ...
  expect(WaitStore.get("wait-session-owner")).toMatchObject({
    status: "resolved",
    partial: false,
    replies: [{ replyKey: scope("reply").id, responderId: "actor-external-worker" }],
  });
});
...
test("duplicate unresolved reply key is refused without attaching twice", async () => {
  ...
  await expect(kernelRouter().ingest(sender, facts("reply"))).rejects.toMatchObject({
    code: "wait_reply_rejected",
    message: "wait reply rejected: duplicate_reply",
  });
...
test("late reply lazily expires the Wait while retaining partial progress", async () => {
  ...
  expect(
    WaitService.attachReply(
      "late",
      { replyKey: "early", responderCandidates: ["b"], at: 1000 },
      "trace",
    ).kind,
  ).toBe("attached");
  await expect(
    ...
    code: "wait_reply_rejected",
    message: "wait reply rejected: deadline_passed",
  });
...
test("unexpected responder is refused with an authoritative route correction", async () => {
  ...
  await expect(
    ...
    code: "wait_reply_rejected",
    message: "wait reply rejected: unknown_responder",
  });
...
test("same-precedence ambiguity is denied before inbox commit", async () => {
  ...
  expect(routingDecisions()[0]).toMatchObject({
    stage: "wait_correlation",
    outcome: "ambiguous",
    candidateInteractionIds: ["wait:a", "wait:b"],
  });
  expect(commits).toEqual([]);
});
...
test.each([
  "ask_clarification",
  "invalid",
])("disallowed Wait action %s never falls through to the surface", async (action) => {
  openWait("disallowed");
...
test("retired owner cannot enter the Wait store", () => {
  expect(() =>
    openWait("retired", { ownerRef: Wait.OwnerRef.parse({ kind: "workItem", id: "retired" }) }),
  ).toThrow();
...
test("awaited send resolves quorum from distinct authenticated responder endpoints", async () => {
  ...
  expect(sent).toMatchObject({
    kind: "sent",
    operation: "awaited",
    wait: { correlation: { endpointId: "telegram:target", replyToMessageId: "platform-message" } },
  });
  for (const externalId of ["responder-1", "responder-2"]) {
    ...
    expect(WaitStore.get("quorum")?.status).toBe(
