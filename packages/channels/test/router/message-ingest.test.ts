...
test("session ingest commits once through the injected inbox without a channel driver", async () => {
	...
	const router = createGatewayRouter({
		...
		prepare: () => ({
			...
			message: {
				sender: "session", senderRole: "resident", targetKind: "session",
				targetRole: "worker", type: "message", parentChild: true,
				fanout: 0, depth: 1, withinParentDeadline: true,
			},
		}),
		run: async (_sender, request, body) => ({
			...
			value: await body({
				action: {
					id: "source", sessionId: "parent", parentId: null, kind: "message",
					intent: { encodingVersion: 1, value: { value: request.intent } }, effect: { encodingVersion: 1, value: {} },
					irreversible: true, ordinal: 1, ts: 1,
				},
				revision: 1,
			}),
		}),
	...
	expect(commits[0]).toMatchObject({
		sessionId: "child", kind: "prompt", content: "work",
		sender: { sessionId: "parent", owner: "process", fence: 1 },
		origin: { value: { kind: "message", senderSessionId: "parent", sourceActionId: "source" } },
	});
});

test.each(["content", "target"] as const)("pre transform of %s is applied or refused before inbox commit", async (field) => {
	const commits: Inbox.Commit[] = [];
	const router = createGatewayRouter({
		sink: () => undefined,
		inbox: {
		...
		prepare: () => ({
			...
			message: {
				sender: "session", senderRole: "resident", targetKind: "session",
				type: "message", parentChild: true, fanout: 0, depth: 1, withinParentDeadline: true,
			},
		}),
		run: async (_sender, request, body) => {
			const value = request.intent;
			if (value === null || typeof value !== "object" || Array.isArray(value)) throw new Error("invalid fixture intent");
			return {
				terminal: "executed", matchedRuleIds: [],
				value: await body({
					action: {
						id: "source", sessionId: "parent", parentId: null, kind: "message",
						intent: {
							encodingVersion: 1, value: {
								value: {
									...value, ...(field === "content" ? { content: "redacted" } : { to: { kind: "session", id: "other" } }),
								}
							}
						},
	...
	const result = router.ingest({ kind: "session", id: "parent" }, {
