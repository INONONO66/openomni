...
test("committed scoped reply authority survives closing storage and constructing a new router source", () => {
	Storage.withIsolation(() => {
		...
		const ports = {
			rules: () => [{
				id: "rule-1", senderId: "persona", surface: "telegram",
				operations: ["fire_and_forget" as const], instanceTtlMs: 60_000,
				maxLiveInstances: 1, createdBy: "owner",
			}],
			publish: () => undefined,
		};
		try {
			...
			createReplyGrantInstances(ports).admit({
				actorId: "guest", endpoint: { channel: "telegram", externalId: "chat-1" },
				surface: "telegram", traceId: "trace-1", at, sourceId: "inbox-1",
			});
			Storage.reset();
			Storage.configure(new SqliteStorageAdapter(path));
			...
			expect(resolveScopedSenderTargetGrant(grants, {
				senderId: "persona", targetActorId: "guest", operation: "fire_and_forget",
				surfaceKey: "telegram:chat-1", at,
			})?.id).toBe("reply-grant:rule-1:inbox-1");
			expect(resolveScopedSenderTargetGrant(grants, {
				senderId: "persona", targetActorId: "guest", operation: "fire_and_forget",
				surfaceKey: "telegram:chat-2", at,
			})).toBeUndefined();
		} finally {
			Storage.reset();
