...
it.each(["auth, secret-token", "other, auth, secret-token"])(
  "negotiates exactly one auth response header for %s on a real Bun server",
  async (protocols) => {
    const handler = new WebSocketHandler(async () => ({ text: "ok" }), () => undefined, {
      token: "secret-token",
    });
    ...
    try {
      ...
      expect(lines.filter((line) => /^sec-websocket-protocol:/i.test(line))
