...
describe("WebSocketHandler authentication", () => {
	...
	it("does not bind an actor on tokenless bootstrap", () => {
		...
		expect(handler.handleUpgrade(new Request("http://localhost/ws?actor=alice"), upgrade.server))
			.toBeUndefined();
		expect(upgrade.options?.data).toMatchObject({ externalId: expect.stringMatching(/^connection:/) });
...
describe("WebSocketHandler ingress and receipts", () => {
	function connection(data: {
		...
		return {
			...
			ws: { data, send: (message: string) => { sent.push(message); framed.resolve(); } },
		};
	}
	...
	it("emits Gateway.IngressFacts and an accepted receipt", async () => {
		...
		const { ws, framed, sent } = connection({ surfaceKey: "ws::dm:c1", authenticated: true, externalId: "connection:c1" });
	...
	it("reconnects without losing the current delivery target", () => {
		...
		const second = connection({ surfaceKey: "ws::dm:c2", authenticated: true, externalId: "alice" });
