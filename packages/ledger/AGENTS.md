...
│   ├── commit-coordinator.ts # SOLE owner of decision-class commit mechanics for Approval, Wait, and WorkItem: append at expectedHead → adopt an empty pre-cutover stream → caller's projection CAS, with SQLITE_BUSY mapped to the caller's typed error. A refused projection is atomic via a nested transaction (savepoint), so head never outruns revision. Domains keep their folds, fact payloads, adoption genesis, and conflict taxonomy
...
│   ├── lifecycle.ts      # start/fail/cancel plus typed raw-completion refusal, evidence
