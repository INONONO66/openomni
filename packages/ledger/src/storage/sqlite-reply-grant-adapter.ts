	...
	return {
		claim(grant, bound) {
			return db.transaction(() => {
				...
				const capacity = db.query<{ count: number }, [string, number]>(
					"SELECT COUNT(*) AS count FROM reply_grant WHERE rule_id = ? AND expires_at >= ?",
				).get(grant.ruleId, bound.at);
				if (capacity !== null && capacity.count >= bound.maxLiveInstances) return "capacity" as const;
				db.query(
					`INSERT INTO reply_grant (id, data, rule_id, target_actor_id, surface_key, expires_at)
           VALUES (?, ?, ?, ?, ?, ?)`,
				).run(grant.id, JSON.stringify(grant), grant.ruleId, grant.targetActorId,
					grant.replyScope.surfaceKey, grant.expiresAt);
				return "claimed" as const;
			}).immediate();
		},
		listLive(at) {
			return db.query<{
				id: string; data: string; rule_id: string; target_actor_id: string;
				surface_key: string; expires_at: number;
			}, [number]>(
				...
				if (grant.id !== row.id || grant.ruleId !== row.rule_id
					|| grant.targetActorId !== row.target_actor_id
					|| grant.replyScope?.surfaceKey !== row.surface_key || grant.expiresAt !== row.expires_at) {
