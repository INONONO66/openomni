...
describe("SqliteStorageAdapter appConnectorInstallation", () => {
  ...
  test("store rejects every consent dimension outside connector requirements", () => {
    ...
    const cases: ReadonlyArray<{
      ...
      {
        name: "unrequested credential",
        consent: { grantedBy: "act_owner", credentials: ["ANTHROPIC_API_KEY"] },
        error: "not requested by connector",
      },
      {
        name: "unrequested capability",
        consent: { grantedBy: "act_owner", capabilities: ["shell"] },
        error: "not requested by connector",
      },
      {
        name: "broader permission",
        consent: {
          grantedBy: "act_owner",
          permissions: [{ action: "tool.call", allowlist: ["*"] }],
        },
        error: "exceeds connector requirement",
      },
      {
        name: "unrequested allow dimension",
        permissions: [{ action: "tool.call", allowLabels: ["capability:read"] }],
        consent: {
          grantedBy: "act_owner",
          permissions: [
            { action: "tool.call", allowLabels: ["capability:read"], allowlist: ["*"] },
          ],
        },
        error: "allowlist exceeds connector requirement",
      },
      {
        name: "omitted allowing field",
        consent: { grantedBy: "act_owner", permissions: [{ action: "tool.call" }] },
        error: "omits connector requirement",
      },
      {
        name: "omitted restrictive field",
        permissions: [{ action: "tool.call", denylist: ["rm.*"] }],
        consent: { grantedBy: "act_owner", permissions: [{ action: "tool.call" }] },
        error: "denylist omits connector requirement",
      },
      {
        name: "weakened restrictive field",
        permissions: [{ action: "tool.call", denylist: ["rm.*"] }],
        consent: { grantedBy: "act_owner", permissions: [{ action: "tool.call", denylist: [] }] },
        error: "denylist omits connector requirement",
      },
      {
        name: "omitted permission actions",
        consent: { grantedBy: "act_owner", capabilities: ["git"] },
        error: "omit connector requirements",
      },
      {
        name: "empty permissions",
        consent: { grantedBy: "act_owner", permissions: [] },
        error: "omit connector requirements",
      },
      {
        name: "one omitted action",
        permissions: [
      ...
      {
        name: "duplicate required action",
        permissions: [
          { action: "tool.call", allowlist: ["git.*"] },
        ...
        consent: {
          grantedBy: "act_owner",
          permissions: [
            { action: "tool.call", allowlist: ["git.*"], allowLabels: ["capability:read"] },
          ],
        },
        error: "duplicate connector requirement",
      },
      {
        name: "duplicate input rule omits another",
        permissions: [
          { action: "tool.call", allowlist: ["bash"], inputRules: [firstRule, secondRule] },
        ],
        consent: {
          grantedBy: "act_owner",
          permissions: [
            { action: "tool.call", allowlist: ["bash"], inputRules: [firstRule, firstRule] },
          ],
        },
        error: "inputRules omits connector requirement",
      },
    ];
      ...
      const registered = AppConnectorInstallationStore.set(
        item.permissions === undefined
          ...
          : {
              ...
              definition: {
                ...record.definition,
                requires: { ...record.definition.requires, permissions: item.permissions },
              },
            },
      );
      AppConnectorInstallationStore.requestConsent(registered.id);
      expect(
        () => AppConnectorInstallationStore.grantConsent(registered.id, item.consent),
        item.name,
      ).toThrow(item.error);
    }
  });
