...
Object.defineProperty(fs, "writeFileSync", {
  value: (...args: Parameters<typeof fs.writeFileSync>) => {
    writeFile(...args);
    if (typeof args[0] === "number" && files.get(args[0])?.includes("archive.sqlite"))
      signal("archive_write");
  },
});
...
Object.defineProperty(fs, "linkSync", {
  value: (...args: Parameters<typeof fs.linkSync>) => {
    link(...args);
...
Object.defineProperty(fs, "rmSync", {
  value: (...args: Parameters<typeof fs.rmSync>) => {
    remove(...args);
    const path = args[0].toString();
    if (restores.delete(path))
      write(1, `${JSON.stringify({ cleanedRestore: path, absent: !fs.existsSync(path) })}\n`);
  },
});
const copy = fs.copyFileSync;
Object.defineProperty(fs, "copyFileSync", {
  value: (...args: Parameters<typeof fs.copyFileSync>) => {
    copy(...args);
    signal("restore_copy");
  },
});
...
Object.defineProperty(Database.prototype, "serialize", {
  value(this: Database, ...args: Parameters<Database["serialize"]>) {
