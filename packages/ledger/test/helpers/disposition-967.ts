...
export function createDispositionFixture(reportCleanup = true) {
  ...
  db.run(
    "INSERT INTO bus_event (event_type, category, data, trace_id, time_created) VALUES ('historical', 'retired', '{}', 'trace', 1)",
  );
  db.run(
    "INSERT INTO session (id, data, time_created, time_updated) VALUES ('legacy', '{}', 1, 1)",
  );
  db.run(
    "INSERT INTO message (id, session_id, data, role, time_created, status) VALUES ('message', 'legacy', '{\"pending\":true}', NULL, 9223372036854775807, 'pending')",
  );
  db.run(
    "INSERT INTO part (id, message_id, data, type, time_start) VALUES ('part', 'message', X'00FF80', NULL, -9223372036854775808)",
  );
  db.run(
    "INSERT INTO action (id, session_id, kind, intent, effect, irreversible, encoding_version, ts, ordinal) VALUES ('attempt-history', 'legacy', 'attempt', '{}', '{}', 1, 1, 1, 1)",
  );
  db.run(
    "INSERT INTO inbox (id, session_id, kind, content, origin, encoding_version, status, time_created, ordinal) VALUES ('pending-inbox', 'legacy', 'prompt', 'pending', '{}', 1, 'pending', 1, 1)",
  );
  db.run(
    "INSERT INTO alarm (id, session_id, kind, fire_at, encoding_version, status, time_created, time_updated) VALUES ('armed-alarm', 'legacy', 'at', 5000000000000, 1, 'armed', 1, 1)",
  );
  db.run(
  ...
  createSqliteWaitAdapter(db).create(
    Wait.Record.parse({
      ...
      correlation: { replyToMessageId: "preserved-platform-receipt" },
...
export function seedRetiredWait(db: Database, status: Wait.Record["status"] = "cancelled") {
  const record = Wait.Record.parse({
    ...
    correlation: { replyToMessageId: "platform-receipt" },
    allowedActions: ["report_result"],
    expectedResponders: ["alice"],
    resolutionPolicy: "first_reply",
    status,
    partial: false,
    replies:
      status === "resolved" ? [{ replyKey: "reply", responderId: "alice", receivedAt: 2 }] : [],
    revision: status === "open" ? 0 : 1,
    expiresAt: 10,
    followUpWindow: 100,
    createdAt: 1,
    updatedAt: status === "expired" ? 11 : 2,
  ...
  const tables = schema
    ...
    .map(({ name }) => {
      const statement = db.prepare<
        Record<string, string | bigint | number | Uint8Array | null>,
        []
      >(`SELECT rowid, * FROM "${name.replaceAll('"', '""')}" ORDER BY rowid`);
      try {
        return { name, rows: statement.all() };
      } finally {
        statement.finalize();
      }
    });
