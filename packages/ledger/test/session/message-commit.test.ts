...
beforeEach(() => {
	...
	SessionHandleStore.acquireLease({
		sessionId: "parent", owner: "sender", expectedFence: 0, now: 10, expiresAt: 100,
	});
});
afterEach(() => {
	Storage.reset();
...
function childMessage() {
	const snapshot = SessionHandleStore.generationSnapshot({
		generation: 1, revertTo: 0, tools: [], system: { preset: "", blocks: [] },
		policyGeneration: 0,
	});
	return {
		id: "letter",
		...
		createSession: LedgerSession.Materialize.parse({
			row: {
				id: "child", parentId: "parent", role: "worker", leaseOwner: null,
				leaseFence: 0, leaseExpiresAt: null, revision: 0, state: "idle",
...
test("child identity and first inbox are visible together at the commit signal", async () => {
	...
	const timeout = setTimeout(() => observed.reject(new Error("missing inbox commit signal")), 10_000);
...
test("stale sender fence refuses child allocation before any write", () => {
	...
	SessionHandleStore.acquireLease({
		sessionId: "parent", owner: "replacement", expectedFence: 1, now: 100, expiresAt: 200,
	});
...
test("message id reuse cannot allocate a second child", () => {
	...
	const reused = {
		...next, sessionId: "second",
		createSession: {
			row: { ...next.createSession.row, id: "second" },
			initialAction: { ...next.createSession.initialAction, id: "second:configure", sessionId: "second" },
		},
	};
...
test("the commit transaction rechecks fanout and releases capacity after terminal", () => {
	...
	const second = {
		...first, id: "second-letter", sessionId: "second",
		createSession: {
			row: { ...first.createSession.row, id: "second" },
			initialAction: { ...first.createSession.initialAction, id: "second-config", sessionId: "second" },
		},
	};
	expect(SessionHandleStore.openChildCount("parent")).toBe(1);
	expect(() => SessionHandleStore.commitInbox(second)).toThrow();
	expect(SessionHandleStore.listRows().map((row) => row.id)).toEqual(["child", "parent"]);
	SessionHandleStore.acquireLease({ sessionId: "child", owner: "worker", expectedFence: 0, now: 30, expiresAt: 100 });
	expect(SessionHandleStore.commit({
		sessionId: "child", owner: "worker", fence: 1, now: 40, expectedRevision: 2,
		consumeInboxIds: ["letter"], state: "idle", releaseLease: true,
		actions: [{ id: "child-result", parentId: "letter", sessionId: "child", kind: "turn", ts: 40, irreversible: true,
			intent: { encodingVersion: 1, value: { phase: "terminal", turnId: "child-turn" } },
			effect: { encodingVersion: 1, value: { phase: "terminal", turnId: "child-turn", kind: "result", text: "done", boundaryActionId: null, resumeCount: 0 } },
		}],
	}).ok).toBe(true);
...
test("a terminal and its parent inbox delivery roll back on the same fault", () => {
	...
	SessionHandleStore.commitInbox(childMessage());
	SessionHandleStore.acquireLease({
		sessionId: "child", owner: "worker", expectedFence: 0, now: 30, expiresAt: 100,
	});
	const terminal: LedgerAction.Append = {
		id: "child:terminal", parentId: "letter", sessionId: "child", kind: "turn",
	...
	const request = {
		sessionId: "child", owner: "worker", fence: 1, now: 40, expectedRevision: 2,
		actions: [terminal], consumeInboxIds: ["letter"], state: "idle" as const, releaseLease: true,
		deliveries: [{
			id: "parent:reply", sessionId: "parent", kind: "prompt" as const, content: "done",
			createdAt: 40, parentActionId: null,
	...
	expect(SessionHandleStore.tree("child").map((action) => action.id)).toEqual(["child:configure", "letter"]);
