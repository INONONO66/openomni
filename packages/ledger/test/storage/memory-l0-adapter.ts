...
import { alarmAppend, inboxAppend, messageAnswerAppend, messageTimeoutInbox } from "../../src/storage/l0-action-builders.js";
...
export function createMemoryL0Adapter(): MemoryL0Adapter {
	...
	function openChildCount(parentId: string): number {
		return [...sessionRows.values()].filter((child) => {
			...
			if ([...inboxRows.values()].some((item) => item.sessionId === child.id && item.status === "pending")) return true;
			const actions = [...actionRows.values()].filter((action) => action.sessionId === child.id && action.kind === "turn");
			...
			const terminal = actions.flatMap((action) => {
				...
				return transaction(() => {
					...
					if (child !== undefined) {
						...
						if (child.row.parentId !== null) {
							...
							if (limits === undefined || openChildCount(child.row.parentId) >= limits.fanout) return undefined;
							let depth = 1;
					...
					if (reply.success) {
						...
						claimMessageAnswer({ sessionId: parsed.sessionId, sourceActionId: reply.data.sourceActionId,
							messageId: reply.data.messageId, at: parsed.createdAt, state: "answered" });
					}
					...
					if (receipt === undefined) {
						if (child !== undefined || reply.success) throw new Error("message inbox commit refused");
						return undefined;
					}
		...
		alarms: {
			...
			fireMessage(id, at) {
				return transaction(() => {
					...
					if (alarm === undefined || alarm.status !== "armed" || at < alarm.fireAt) return undefined;
					...
					const current = owner.toolsGeneration === pinned.toolsGeneration && owner.systemHash === pinned.systemHash && owner.policyGeneration === pinned.policyGeneration;
					const receipt = appendMemoryAction(sessionRows, actionRows, {
					...
					if (!current || !claimMessageAnswer({ sessionId: alarm.sessionId, sourceActionId: spec.sourceActionId,
						messageId: spec.messageId, at, state: "timed_out" })) return undefined;
