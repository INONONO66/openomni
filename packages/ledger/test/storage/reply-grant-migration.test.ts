...
import { initializeSqliteDatabase, preflightSqliteDatabase } from "../../src/storage/sqlite-schema-lifecycle";
...
test("0035 upgrade and fresh 0036 schema share the measured fingerprint and reopen", () => {
	...
	expect(snapshotDatabase(fixture.db).tables.filter(({ name }) => !["_migrations", "reply_grant"].includes(name)))
...
test("the old archive allows only an empty pinned forward projection", () => {
	...
	expect(() => assertArchiveEquality(fixture.db, archived, true)).toThrow("stale_archive:reply_grant");
});
