...
- **SDK wiring** (`provider/sdk.ts`): `getSDK(model, auth)` resolves bundled Anthropic/OpenAI or an explicit OpenAI-compatible endpoint. SDK and language-model caches are bounded LRU maps; keys include provider/package/URL and a SHA-256 auth fingerprint, while the language cache additionally includes model id. Raw credentials never appear in cache keys. Provider-specific behavior belongs in `provider/`, `auth/`, or `transform/`, not in call sites.
