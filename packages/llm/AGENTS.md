...
├── sink.ts           # Streaming callback contract: message/tool projections plus optional transcript facts
...
- **Processor**: Created via `Processor.create({ assistantMessage, sessionID, model, abort, trace, events, sink?, createStream, toolNames?, maxRetryAttempts? })`. `trace` and `events` are required. `process({ system })` resolves on stream completion and throws on abort/terminal error; `run()` maps that to `Run.Outcome` and publishes one terminal LLM event for every start. Each retry gets a fresh transcript attempt id; failed attempts close before the next begins, while usage totals include all billed attempts. Folded snapshots are immutable and emitted at part boundaries, not per token delta. Tool names are provider-safe on the wire and mapped back to their original dotted names in transcript facts. Processor owns tool stream projection; tool execution happens only in `run()`'s AI SDK callback. Operational status is `busy` → (`retry`…) → `idle`, exactly one `idle` per process call.
