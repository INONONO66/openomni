...
Provider-boundary quality update: 2026-09-09. Single-attempt ownership descriptions below match #937.
...
├── processor/
│   ├── index.ts      # Processor.create() — one attempt, transcript fold, sink/event projection
...
└── model/
    ├── index.ts      # ModelsDev.get — internal catalog owner; loader.ts/schema.ts decode cache/fetch/snapshot
