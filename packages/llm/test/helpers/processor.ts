...
export function streamOf(chunks: readonly StreamEvent[]): Processor.ProcessorOptions["createStream"] {
  return async () => ({ fullStream: (async function* () { yield* chunks; })() });
}
...
export function capturingSink() {
  ...
  return { sink, messages, toolCalls, toolResults, textTimeline, finalParts: () => messages.at(-1)?.parts ?? [] };
}

export function processorInfo(events: ReturnType<typeof collector>) {
  return events.named(Operational.Events.Info.name)
...
export function statusStates(events: ReturnType<typeof collector>): string[] {
  return processorInfo(events).filter((event) => event.msg === "sink.snapshot")
...
export function useProcessor() {
  ...
  return {
    events,
    get assistantMessage() { return message; },
    get abortController() { return abort; },
    createProcessor(overrides: Partial<Processor.ProcessorOptions> = {}): ReturnType<typeof Processor.create> {
