...
const tool = { name: "test_tool", description: "A test tool", inputSchema: { type: "object" as const, properties: { x: { type: "string" as const } } } };
...
describe("run schema-only tools and cache breakpoints", () => {
  ...
  test("maps the advertised schema without installing an execution closure", async () => {
    ...
    expect(capture.args.tools.test_tool).toMatchObject({
      type: "function", description: tool.description,
      inputSchema: { jsonSchema: tool.inputSchema }, providerOptions: breakpoint,
    });
    expect(capture.args.tools.test_tool?.execute).toBeUndefined();
  });

  test("marks only the last tool definition", async () => {
    await capture.run({ tools: [{ ...tool, name: "first" }, { ...tool, name: "last" }] });
  ...
  test("does not add Anthropic breakpoints for OpenAI models", async () => {
    await capture.run({
      tools: [tool], model: { id: "gpt-4o", providerID: "openai", name: "GPT", api: { npm: "@ai-sdk/openai" } },
    });
  ...
  test("marks the system message with the Anthropic breakpoint", async () => {
    ...
    expect(capture.args.messages[0]).toEqual({ role: "system", content: "system fixture", providerOptions: breakpoint });
  });
});
