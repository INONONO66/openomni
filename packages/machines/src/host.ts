...
export async function createMachineHost(options: MachineHostOptions): Promise<MachineHost> {
  ...
  function detach(connectionId: string, reason: string): void {
    ...
    options.events.publish(Machine.Events.Detached, {
      machineId: attachment.offer.machineId,
      time: options.now(),
      reason,
    });
  }
  ...
  const server: IpcServer = await createIpcServer(
    ...
    async (method, params, respond, _notify, connectionId) => {
      ...
      respond({
        status: "attached",
        effectiveCapabilities: [...outcome.capabilities],
        effectiveExports: [...exports.exports],
      } satisfies Machine.AttachResult);
    },
  ...
  function connection(id: string): { id: string; attachment: Attachment } {
    ...
    if (connectionId === undefined || attachment === undefined)
      throw new MachineRefusalError({
        reason: "machine_not_attached",
        message: `machine is not attached: ${id}`,
      });
  ...
  function location(id: string, path: string) {
    ...
    const candidates = (peer.attachment.offer.exports ?? [])
      ...
      .filter((entry) => {
        return (
          absolute === entry.path ||
          absolute.startsWith(entry.path === "/" ? "/" : `${entry.path}/`)
        );
      })
    ...
    if (root === undefined)
      throw new MachineRefusalError({
        ...
        message: "path is outside offered exports",
      });
    if (candidates[1] !== undefined && candidates[1].path === root.path)
      throw new MachineRefusalError({
        reason: "ambiguous_export",
        message: "multiple exports name the same root",
      });
    return { connectionId: peer.id, export: root.name, path: posix.relative(root.path, absolute) };
  }

  async function filesystem<O extends Machine.FsValue["op"]>(
    id: string,
    path: string,
    op: O,
    extra: { data?: string; offset?: number; limit?: number } = {},
  ): Promise<Value<O>> {
    const target = location(id, path);
    const request = Machine.FsRequest.parse({
      op,
    ...
    const result = Machine.FsResult.parse(
      await typedCall(server, Machine.WireMethod.FsOp, request),
    );
    if (result.status === "refused") throw new MachineRefusalError(result);
    if (result.value.op !== op)
      throw new MachineRefusalError({
        reason: "invalid_response",
        message: "filesystem response operation mismatch",
      });
  ...
  function get(id: string): MachineHandle {
    ...
    const handle: MachineHandle = {
      fs: {
        ...
        write: (path, data) =>
          filesystem(id, path, "write", { data: Buffer.from(data).toString("base64") }),
      ...
      async exec(cmd, cwd) {
        ...
        const result = Machine.ExecResult.parse(
          await typedCall(
            server,
            Machine.WireMethod.Exec,
            Machine.ExecRequest.parse({ cmd, cwd }),
            Machine.EXEC_TIMEOUT_MS + 1000,
          ),
        );
      ...
      async runCode(cell, signal) {
        ...
        const cancel = () => {
          ...
          server.useConnection(peer.id);
          cancellation = typedCall(server, Machine.WireMethod.CancelCode, {
            cellId: request.cellId,
          }).then(
            () => undefined,
            (error: Error) => {
        ...
        try {
          server.useConnection(peer.id);
          result = Machine.CellResult.parse(
            await typedCall(server, Machine.WireMethod.RunCode, request, request.timeoutMs + 1000),
          );
        } finally {
  ...
  return {
    ...
    list: () =>
      [...attachments.values()]
        .map(({ enrollment, offer, capabilities }) => ({
          ...structuredClone(enrollment),
          tags: [...(enrollment.tags ?? [])],
          capabilities: [...capabilities],
          os: offer.platform.split("-")[0] ?? offer.platform,
          arch: offer.platform.split("-").slice(1).join("-"),
        }))
