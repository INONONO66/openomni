...
Orchestration layer for `@openomni/openomni`. Builds on `@openomni/agent`, `@openomni/session`, and `@openomni/protocol` to add DAG utilities, inbound event handling, dispatch, Resident runtime, and worker execution tooling. This package is the future home for Resident orchestration seams: controlled inbound authority, self-loop session creation, Worker delegation, and distilled writeback.
