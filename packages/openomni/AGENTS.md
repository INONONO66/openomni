...
- `src/execution-runtime/tool/agent/tools/dispatch.ts` is the `dispatch` tool — the cross-session orchestration gate. All worker spawn/send/resume/cancel, resident delivery, and schedule create/cancel actions route through `Dispatch.submit()`, which enforces PolicyEngine authorization and emits Bus audit events. See `src/dispatch/` for the runtime, handlers, and policy.
