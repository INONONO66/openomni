...
| `teammate.ts` | Creates a fresh `ChatAgent` per step execution. No cross-step state; each call is isolated. Merges config-level and step-level tools (step takes precedence). |
...
| `run-ledger.ts` | In-memory step state tracker. Manages `ready -> running -> succeeded/failed` transitions, attempt counts, and rejection streaks. No persistence; state lives only for the duration of a single orchestration run. |
...
RunLedger is in-memory only. It tracks per-step state (`ready`, `running`, `succeeded`, `failed`, `skipped`), attempt count, and rejection streak. State transitions are validated against a transition table; invalid transitions throw. The ledger returns defensive copies from all read methods to prevent external mutation.
