...
Ring-1 pure target-selection package (docs/architecture.md § Outbound target selection). `Placement.selectModel(chain, priorFailureReasons)` folds an ordered model-candidate chain and decided failure history into the next model. `Placement.resolveTools(tools, targets)` folds a tool catalog and caller-supplied host/machine effective capability sets into offerability decisions and eligible machine ids. Both are pure protocol-fold decisions (no clock, no store, no I/O; deterministic and replayable).
...
- Tool placement reads absent `Tool.Spec.placement` as `free` in exactly one place, requires one candidate to hold the complete `requires` subset, and returns machine ids in lexical order. Effective sets come from protocol's `Machine.effectiveCapabilities`; placement never repeats enrollment/offer negotiation.
...
`packages/agent` — `runAgent` resolves the model per retry attempt through the model fold. `buildTurn` resolves the configured catalog through the machine fold before `tool.catalog.pre`, prompt tool fragments, and the llm seam; `ChatAgentConfig.toolTargets` carries the effective capability facts.

## TESTS

`bun test --timeout 15000` in this package: model advancing/non-advancing classes, clamping + exhaustion, mixed histories, and empty-chain refusal; tool placement × requirements, unknown capabilities, complete-subset matching, multiple eligible machines, deterministic ordering, and empty-target refusal.
