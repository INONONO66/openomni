...
function parseRow(row: PolicyRow.Row, generation: number, kinds: ReadonlySet<string>): CompiledRow {
	...
	if (
		...
		throw new PolicyCompileError({
			code: "invalid_verdict", generation, ruleName: row.name, kind: row.kind, phase: row.phase,
		});
	}
	return Object.freeze({
		name: row.name,
...
function evaluateSnapshot(
	...
	const missingMessageContext = input.kind === "message" && input.op === "sendMessage" && input.message === undefined;
