...
export namespace Inbox {
	export const ReplyOrigin = z.object({
	...
	export const Commit = Row.omit({
		...
		limits: z.object({ fanout: z.number().int().nonnegative(), depth: z.number().int().nonnegative() }).strict().optional(),
	}).refine((row) => row.createSession === undefined || row.createSession.row.parentId === null || row.limits !== undefined, {
		path: ["limits"], message: "child materialization requires pinned admission limits",
	});
	export type Commit = z.infer<typeof Commit>;
...
export namespace Alarm {
	export const MessageDeadline = z.object({
