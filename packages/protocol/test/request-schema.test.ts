...
const original: SessionTransition.Request = {
  ...
  callId: "call",
  mode: "approval",
  parsedInput: { personId: "owner" },
  inputHash: canonicalDigest({ personId: "owner" }),
  effectHash: canonicalDigest({ category: "authority" }),
  generation: 1,
  toolsGeneration: 1,
  toolsHash: "catalog",
  systemHash: "system",
  domainRevisions: { person: 4 },
  deadline: 200,
...
test("a request preserves the original invocation and rejects incomplete or contradictory terminals", () => {
  ...
  expect(
    SessionTransition.Request.safeParse({ ...original, state: "expired", outcome: "answered" })
      .success,
  ).toBe(false);
  expect(SessionTransition.Request.safeParse({ ...original, outcome: "answered" }).success).toBe(
    false,
  );
...
test("request quorum and all bounds count distinct expected responders", () => {
  for (const resolution of ["quorum", "all"] as const) {
    expect(
      SessionTransition.Request.safeParse({
        ...original,
        resolution,
        threshold: 2,
        expectedResponders: ["one", "two"],
      }).success,
    ).toBe(true);
    expect(
      SessionTransition.Request.safeParse({
