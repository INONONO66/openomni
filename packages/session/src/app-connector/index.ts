...
export namespace AppConnectorInstallationStore {
  ...
  export function grantConsent(id: string, input: ConsentInput): AppConnector.Installation {
    ...
    assertRequestedValues(
      input.credentials,
      installation.definition.requires.credentials,
      "credentials",
    );
    assertRequestedValues(
      input.capabilities,
      installation.definition.requires.capabilities,
      "capabilities",
    );
    assertRequestedPermissions(input.permissions, installation.definition.requires.permissions);
