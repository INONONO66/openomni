...
describe("SqliteStorageAdapter — plan sub-adapter", () => {
  let dbPath = "";
  let adapter: SqliteStorageAdapter;
  ...
  test("read: returns undefined for non-existent", async () => {
    expect(await adapter.plan?.read("missing")).toBeUndefined();
  });

  test("write and read: roundtrip", async () => {
    await adapter.plan?.write("plan-1", "# My Plan");
    const result = await adapter.plan?.read("plan-1");
  ...
  test("write: second write increments version", async () => {
    await adapter.plan?.write("plan-1", "v1");
    await adapter.plan?.write("plan-1", "v2");
    const result = await adapter.plan?.read("plan-1");
    expect(result?.content).toBe("v2");
    expect(result?.version).toBe(2);
  });

  test("write: third write reaches version 3", async () => {
    await adapter.plan?.write("p1", "a");
    await adapter.plan?.write("p1", "b");
    await adapter.plan?.write("p1", "c");
    expect((await adapter.plan?.read("p1"))?.version).toBe(3);
  });

  test("delete: removes plan", async () => {
    await adapter.plan?.write("plan-1", "content");
    await adapter.plan?.delete("plan-1");
    expect(await adapter.plan?.read("plan-1")).toBeUndefined();
  });

  test("delete: no-op for non-existent", async () => {
    await expect(adapter.plan?.delete("missing")).resolves.toBeUndefined();
  });

  test("list: returns empty when no plans", async () => {
    expect(await adapter.plan?.list()).toEqual([]);
  });

  test("list: returns all plans with metadata", async () => {
    await adapter.plan?.write("p1", "content1");
    await adapter.plan?.write("p2", "content2");
    const list = await adapter.plan?.list();
  ...
  test("list: does not include content", async () => {
    await adapter.plan?.write("p1", "secret content");
    const list = await adapter.plan?.list();
    expect(list?.[0]).not.toHaveProperty("content");
  });

  test("list: ordered by time_created ascending", async () => {
    await adapter.plan?.write("p1", "first");
    await new Promise((r) => setTimeout(r, 5));
    await adapter.plan?.write("p2", "second");
    const list = await adapter.plan?.list();
