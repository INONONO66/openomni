...
`Bus`, `newTraceId`, and `newSpanId` have production consumers. Everything else — `span`, `child`, the sinks (`tee`, `noopSink`, `collector`), the `traceparent` codec, and the guards (`requireTraceScope`, `rootScope`, `isTraceId`, `isSpanId`) — is used only by this package's own tests, and `scope` only by a `packages/session` test fixture. `createSpanHandle` and `failedOutcome` are not exported from the barrel at all: `scope.ts` is their only consumer, and an export with no reader is the thing this rule exists to catch. `check-dead-exports` is satisfied because tests count as consumers, so it will not tell you this.
