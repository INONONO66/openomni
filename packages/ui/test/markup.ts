...
export function attributes(node: ReactNode, selector = "*") {
  ...
  new HTMLRewriter().on(selector, {
    element(element) {
      elements.push(Object.fromEntries([...element.attributes].map(([name, value]) => [name, unescapeAttribute(value)])));
    },
...
export function textContent(node: ReactNode, selector: string): string {
  const chunks: string[] = [];
  new HTMLRewriter().on(selector, { text(chunk) { chunks.push(chunk.text); } })
    .transform(renderToStaticMarkup(node));
  return unescapeAttribute(chunks.join(""));
}

export function classes(node: ReactNode, selector = "*"): string[] {
  return attributes(node, selector).flatMap((element) => (element.class ?? "").split(/\s+/).filter(Boolean));
}
