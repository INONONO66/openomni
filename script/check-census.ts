...
import {
  ledgerCensusOwner,
  ledgerCensusRole,
  ledgerCensusSchemaCompiler,
  ledgerCensusSchemaOrigins,
  liveCensusTables,
...
type Edge = {
  ...
  importOrAliasPath: Locus[];
  forwardingCallPath: Locus[];
  terminalProductOperation: Locus;
  rootInvocation: Locus;
...
function exact(value: { [key: string]: Json }, keys: string[]): void {
  if (Object.keys(value).sort().join() !== keys.sort().join())
    fail("schema", "", "unexpected or missing fields");
}
...
function decode(text: string): Json {
  try {
    ...
    return fail("schema", "", "invalid JSON");
  }
}
...
function sourcePath(root: string, path: string): string {
  if (!path || isAbsolute(path) || path.includes("\\") || path.split("/").includes(".."))
    fail("schema", path, "relative path required");
  const absolute = resolve(root, path);
  if (relative(root, realpathSync(absolute)).startsWith(".."))
...
function readEntry(value: Json): Entry {
  try {
    ...
    return fail("schema", "", "invalid source entry");
  }
}
...
function loadInput(root: string, inventoryPath: string, hash: string, contractPath: string) {
  ...
  if (
    inventory.version !== 1 ||
    contract.version !== 1 ||
    contract.typescript !== "5.9.2" ||
    typeof contract.topology !== "boolean"
  )
    fail("schema", contractPath, "unsupported contract");
  const roots = array(contract.roots).map(string),
    projects = array(contract.projects).map(string);
  ...
  if (inventory.contractHash !== digest(JSON.stringify(normalized)))
    fail("tamper", contractPath, "contract hash differs");
  const files = array(inventory.files).map(readEntry),
    historical = array(inventory.historical).map(readEntry);
  if (!files.length || !projects.length || !roots.length)
    fail("incomplete_inventory", "", "empty source/project inventory");
  const names = new Set<string>();
  for (const row of [...files, ...historical]) {
    if (names.has(row.path)) fail("incomplete_inventory", row.path, "duplicate source");
    names.add(row.path);
    if (checked(root, row.path, row.sha256).length !== row.bytes)
      fail("tamper", row.path, "byte count differs");
  }
  for (const row of files)
    if (!roots.some((prefix) => row.path.startsWith(`${prefix}/`)))
      fail("schema", row.path, "source outside ownership roots");
  const configurations = array(inventory.configurations).map((item) => {
    const config = object(item);
    exact(config, ["path", "sha256"]);
    const path = string(config.path);
    checked(root, path, string(config.sha256));
    return path;
  });
  for (const project of projects)
    if (!configurations.includes(project))
      fail("incomplete_inventory", project, "missing project configuration");
  ...
  return {
    ...
    embedded,
    embeddedText,
    inventoryHash: digest(raw),
    contractHash: string(inventory.contractHash),
  };
}
function loadEmbedded(inventory: { [key: string]: Json }, names: Set<string>, root: string) {
  ...
  for (const row of embedded) {
    ...
    if (!host || !symbol || extra || row.language !== "python" || !names.has(host))
      fail("schema", row.path, "invalid embedded source identity");
    const source = ts.createSourceFile(
      host,
      readFileSync(sourcePath(root, host), "utf8"),
      ts.ScriptTarget.Latest,
      true,
    );
    const matches: string[] = [];
    walk(source, (node) => {
      if (!ts.isVariableDeclaration(node) || node.name.getText() !== symbol || !node.initializer)
        return;
      const value = node.initializer;
      if (
        ts.isTaggedTemplateExpression(value) &&
        value.tag.getText() === "String.raw" &&
        ts.isNoSubstitutionTemplateLiteral(value.template) &&
        value.template.rawText !== undefined
      )
        matches.push(value.template.rawText);
    });
    ...
    if (
      ...
      Buffer.byteLength(text) !== row.bytes
    )
      fail("tamper", row.path, "embedded source differs");
...
function makeProgram(root: string, files: Entry[], projects: string[], shared: CensusPrograms) {
  ...
  if (object(readJson(bunTypes)).version !== "1.3.12")
    fail("tool_version", bunTypes, "Bun declarations require 1.3.12");
  const options: ts.CompilerOptions = {
    types: ["bun"],
    typeRoots: [dirname(dirname(bunTypes))],
    strict: true,
  ...
  if (syntax.length)
    fail("unsupported_syntax", syntax[0]?.file?.fileName ?? "", "source syntax is invalid");
  for (const source of program.getSourceFiles()) {
    if (
      !source.isDeclarationFile &&
      !source.fileName.endsWith(".json") &&
      !source.fileName.includes("/node_modules/") &&
      !membership.has(resolve(source.fileName))
    )
      fail(
        "incomplete_inventory",
        relative(root, source.fileName),
        "resolved source omitted from inventory",
      );
  }
  return program;
}
    ...
    const bins =
      typeof config.bin === "string"
        ...
        : config.bin === undefined
          ? []
          : Object.values(object(config.bin)).map(string);
    for (const bin of bins) {
      const path = resolve(dirname(manifest), bin);
      if (!files.some((row) => resolve(root, row.path) === path && eligible(row)))
        fail(
          "root_classification",
          bin,
          "bin must resolve to inventoried source, not unverified generated output",
        );
  ...
  function discoverWorkflowRoots() {
    if (existsSync(workflow)) {
      ...
      for (const match of content.matchAll(
        ...
        const path = match[1] ?? "";
        if (qualityOnly && files.some((row) => row.path === path && row.category === "tooling"))
          continue;
        if (!files.some((row) => row.path === path && eligible(row)))
          fail("incomplete_inventory", path, "CI operational root absent");
        entries.set(resolve(root, path), {
    ...
    function discoverScriptRoots() {
      for (const [name, command] of Object.entries(scripts)) {
        ...
        for (const match of string(command).matchAll(
          /(?:^|[\s;&])(?:bun|node|tsx|python3?)\s+(?:run\s+)?(?:--[\w-]+\s+)*([\w./-]+\.(?:[cm]?[jt]sx?|py))(?=\s|$)/g,
        )) {
          ...
          if (!row)
            fail(
              "incomplete_inventory",
              relative(root, path),
              "operational entry missing from inventory",
            );
          if (qualityOnly && row.category === "tooling") continue;
          if (!eligible(row))
            fail("root_classification", row.path, "nonproduction operational entry");
          entries.set(path, {
            path: relative(root, manifest),
            line: 1,
            symbol: `scripts.${name}: ${command}`,
          });
        }
      }
    }
      ...
      if (
        Object.values(scripts).some(
          (command) =>
            ...
            /(?:^|[\s;&])electron-vite\s+(?:dev|build|preview)(?:\s|$)/.test(command),
        )
      ) {
        const configuration = files.find(
          (row) => resolve(root, row.path) === join(dirname(manifest), "electron.vite.config.ts"),
        );
        if (!configuration)
          fail(
            "missing_roots",
            relative(root, manifest),
            "electron-vite configuration missing from inventory",
          );
        const file = ts.createSourceFile(
          configuration.path,
          readFileSync(sourcePath(root, configuration.path), "utf8"),
          ts.ScriptTarget.Latest,
          true,
        );
        entries.set(resolve(root, configuration.path), {
          path: relative(root, manifest),
          line: 1,
          symbol: "electron-vite",
        });
        walk(file, (node) => {
          if (!ts.isPropertyAssignment(node) || !["entry", "input"].includes(memberName(node.name)))
            return;
          if (!ts.isStringLiteralLike(node.initializer))
            fail(
              "dynamic_application_entry",
              configuration.path,
              "electron-vite entry must be concrete",
            );
          ...
          for (const path of paths) {
            if (!files.some((row) => resolve(root, row.path) === path && eligible(row)))
              fail(
                "root_classification",
                relative(root, path),
                "application entry absent from inventory",
              );
...
class Provenance {
  ...
  private readonly invocationSites = new Map<
    ts.Node,
    Map<
      ts.CallExpression | ts.NewExpression,
      { arguments: readonly ts.Expression[]; synchronous: boolean }
    >
  >();
  readonly externalEvents = new Map<
    ts.CallExpression,
    {
      source: Locus;
      events: string[];
  ...
  readonly dependencyContracts = new Map<
    string,
    { version: string; sha256: string; operations: Locus[] }
  >();
  constructor(
    readonly root: string,
    readonly program: ts.Program,
    readonly files: Entry[],
    readonly roots: Map<string, Locus>,
    qualityOnly = false,
  ) {
    this.checker = program.getTypeChecker();
    this.sourceFiles = files
      .filter((row) => eligible(row) && (!qualityOnly || qualitySource(row.path)))
      .map((row) => program.getSourceFile(resolve(root, row.path)))
  ...
  private validateEventRegistrations() {
    const validateRegistration = ([call, receiver]: [ts.CallExpression, ts.Expression]): void => {
      ...
      const values = this.runtimeValues(
        receiver,
        new Map<ts.ParameterDeclaration, ts.Expression>(),
      );
      const local = values.some(
        (value) =>
          ts.isNewExpression(value) &&
          (/@types\/node\/events\.d\.ts$/.test(this.nativeOwner(value)) ||
            memberName(value.expression) === "AbortController"),
      );
      ...
      const names = call.arguments[0]
        ? this.strings(call.arguments[0]).map((row) => row.value)
        : [];
      if (!names.length) this.problem(call, "dynamic_event_trigger");
      if (
        native &&
        names.some((name) => native.declared.has(name) && !native.events.has(name)) &&
        !this.externalEvents.has(call) &&
        !this.emittedRegistrations.has(call)
      )
        this.problem(call, "unsupported_native_event_lifecycle", receiver);
      for (const [emission, emittedReceiver] of this.eventEmissions) {
        const emittedNames = emission.arguments[0]
          ? this.strings(emission.arguments[0]).map((row) => row.value)
          : [];
        if (
          !emittedNames.length &&
          memberName(emission.expression) !== "abort" &&
          this.runtimeValues(
            emittedReceiver,
            new Map<ts.ParameterDeclaration, ts.Expression>(),
          ).some((value) => values.includes(value))
        )
          this.problem(emission, "dynamic_event_trigger");
        if (
          emittedNames.some((name) => names.includes(name)) &&
          this.runtimeValues(
            emittedReceiver,
            new Map<ts.ParameterDeclaration, ts.Expression>(),
          ).some((value) => values.includes(value)) &&
          !this.eventContexts(call).some((before) =>
            this.eventContexts(emission).some((after) => before.sites[0] === after.sites[0]),
          )
        )
          this.problem(call, "unresolved_event_order", emission);
      }
    };
  ...
  nativeOwner(call: ts.CallExpression | ts.NewExpression): string {
    return (
      this.checker.getResolvedSignature(call)?.declaration?.getSourceFile().fileName ??
      this.declaration(call.expression)?.getSourceFile().fileName ??
      ""
    );
  }
  locus(node: ts.Node, symbol = node.getText().slice(0, 160)): Locus {
    ...
    return {
      path: relative(this.root, source.fileName),
      line: source.getLineAndCharacterOfPosition(node.getStart()).line + 1,
      symbol,
    };
  }
  problem(node: ts.Node, code: string, value?: ts.Node): void {
    this.errors.push({
      ...this.locus(node),
      code,
      ...(ts.isCallExpression(node) ? { declarationOwner: this.nativeOwner(node) } : {}),
      ...(value
        ? { valueOrigins: [...(this.points.get(value) ?? [])].map((node) => this.locus(node)) }
        : {}),
    });
  }
      ...
      if (ts.isVariableDeclaration(owner) && owner.initializer) {
        ...
        if (value && ts.isObjectLiteralExpression(value)) {
          ...
          if (property && ts.isPropertyAssignment(property))
            return this.resolve(property.initializer, seen);
        }
  ...
  callTarget(call: ts.CallExpression): ts.Node | undefined {
    ...
    if (
      ...
      memberName(target.expression) === "bind" &&
      ts.isPropertyAccessExpression(target.expression)
    ) {
      if (target.arguments.length > 1) {
        this.problem(call, "unsupported_bound_arguments");
        return undefined;
      }
  ...
  private add(from: ts.Node, target: ts.Node, site: ts.Node): void {
    ...
    const values = this.links.get(from) ?? [];
    values.push({ target, site });
    this.links.set(from, values);
  }
  private linkNode(node: ts.Node): void {
      ...
      if (ts.isImportDeclaration(node) || ts.isExportDeclaration(node)) {
        ...
        if (target && ts.isSourceFile(target)) this.add(node.getSourceFile(), target, node);
          ...
          if (module.text.endsWith(".css") && ts.isImportDeclaration(node) && !node.importClause) {
            const path = relative(
              this.root,
              resolve(dirname(node.getSourceFile().fileName), module.text),
            );
    ...
    if (
      ...
      node.initializer &&
      [ts.SyntaxKind.Identifier, ts.SyntaxKind.PropertyAccessExpression].includes(
        unwrap(node.initializer).kind,
      )
    )
  ...
  private linkDynamicModule(node: ts.CallExpression, name: string): void {
    if (!name.startsWith(".") && !name.startsWith("/")) {
      ...
      if (workspace) {
        ...
        if (source) {
          this.add(scope(node), source, node);
          this.point(node, source);
        }
      }
      this.moduleCalls.add(node);
      return;
    }
    const absolute =
      name.startsWith("/") && !name.startsWith(this.root)
        ? resolve(this.root, name.slice(1))
        : resolve(dirname(node.getSourceFile().fileName), name);
    const source =
      this.program.getSourceFile(absolute) ??
      this.program.getSourceFile(absolute.replace(/\.js$/, ".ts")) ??
      this.program.getSourceFile(`${absolute}.ts`) ??
      this.program.getSourceFile(`${absolute}/index.ts`);
    if (source) {
      this.add(scope(node), source, node);
      this.point(node, source);
      this.moduleCalls.add(node);
  ...
  private moduleNames(argument: ts.Expression): string[] {
    ...
    if (
      ...
      memberName(resolved.expression) === "resolveSync" &&
      /bun-types\//.test(this.nativeOwner(resolved))
    ) {
      const name = resolved.arguments[0];
      return name ? this.values(name).map((row) => row.value) : [];
    }
    const concrete = this.strings(argument, true);
    if (concrete.length && concrete.every((row) => !/[?*{]/.test(row.value)))
      return concrete.map((row) =>
        row.value.startsWith(".") || isAbsolute(row.value) ? row.value : `/${row.value}`,
      );
  ...
  private globModuleNames(argument: ts.Expression): string[] {
    ...
    if (
      ...
      !ts.isPropertyAccessExpression(iterable.expression) ||
      memberName(iterable.expression) !== "scan"
    )
      return [];
    const glob = this.declaration(iterable.expression.expression);
    if (
      !glob ||
      !ts.isVariableDeclaration(glob) ||
      !glob.initializer ||
      !ts.isNewExpression(glob.initializer)
    )
      return [];
    ...
    return this.values(pattern).flatMap((row) => {
      ...
      return this.files
        .filter((file) => eligible(file) && matcher.match(file.path))
        .map((file) => `${argument.head.text}${file.path}`);
    });
  }
  private point(node: ts.Node, value: ts.Node): void {
    ...
    values.add(value);
    this.points.set(node, values);
    for (const target of this.transfers.get(node) ?? [])
      this.queue.push(() => this.point(target, value));
  ...
  private flow(from: ts.Node, to: ts.Node): void {
    ...
    targets.add(to);
    this.transfers.set(from, targets);
  ...
  private watch(node: ts.Node, operation: () => void): void {
    ...
    watchers.add(operation);
    this.watchers.set(node, watchers);
  ...
  private activate(node: ts.Node, path: { root: Locus; chain: Locus[] }): void {
    ...
    for (const link of this.links.get(node) ?? []) {
      if (!this.reachable.has(link.target))
        this.activate(link.target, {
          root: path.root,
          chain: [...path.chain, this.locus(link.site)],
        });
    }
  }
            ...
            (value) =>
              ...
              value.properties.every(
                (property) =>
                  ...
                  property.name &&
                  memberName(property.name) !== receiver.name.text,
              ),
          )
        )
          return true;
  ...
  private activeConditional(parent: ts.Node, child: ts.Node, node: ts.Node): boolean {
    ...
    if (
      (expression.kind === ts.SyntaxKind.FalseKeyword && child === parent.thenStatement) ||
      (expression.kind === ts.SyntaxKind.TrueKeyword && child === parent.elseStatement)
    )
      return false;
    if (
      expression.getText() === "import.meta.main" &&
      !this.roots.has(resolve(node.getSourceFile().fileName)) &&
  ...
  private testOnlyFlag(expression: ts.Node): boolean {
    if (
      ...
      memberName(expression.expression) !== "has"
    )
      return false;
    ...
    if (
      !flag ||
      !ts.isStringLiteral(flag) ||
      !/^--(?:self-test(?:-[\w-]+)?|update|fixture)$/.test(flag.text) ||
      !receiver ||
      !ts.isVariableDeclaration(receiver) ||
      !receiver.initializer ||
      !ts.isNewExpression(receiver.initializer)
    )
      return false;
    if (
      receiver.initializer.expression.getText() !== "Set" ||
      !receiver.initializer.arguments?.some((argument) =>
        argument.getText().includes("process.argv"),
      )
    )
      return false;
    return true;
  ...
  private inScope(node: ts.Node, operation: () => void): void {
    const owner = scope(node),
      watchers = this.scopeWatchers.get(owner) ?? [];
    const execute = () => {
      if (this.activeBranch(node)) operation();
    };
    watchers.push(execute);
    this.scopeWatchers.set(owner, watchers);
  ...
  private flowPropertyValue(value: ts.Node, key: string, result: ts.Node): void {
    if (ts.isObjectLiteralExpression(value)) {
      this.flowObjectProperties(value, key, result);
      return;
    }
    if (ts.isArrayLiteralExpression(value) && /^\d+$/.test(key)) {
      const element = value.elements[Number(key)];
      if (element) this.flow(element, result);
    }
    if (ts.isSourceFile(value) || ts.isModuleDeclaration(value)) {
      const module = this.checker.getSymbolAtLocation(
        ts.isModuleDeclaration(value) ? value.name : value,
      );
      const symbol =
        module && this.checker.getExportsOfModule(module).find((row) => row.name === key);
  ...
  private flowObjectProperties(
    ...
    for (const property of value.properties) {
      if (ts.isSpreadAssignment(property)) {
        this.propertyOnce(property.expression, key, result);
        continue;
      }
  ...
  private flowConstructedProperty(value: ts.NewExpression, key: string, result: ts.Node): void {
    ...
    if (declaration && ts.isClassDeclaration(declaration)) {
      for (const member of declaration.members)
        if (member.name && memberName(member.name) === key) this.flow(member, result);
    }
  }
  ...
  private propertyOnce(base: ts.Node, key: string, result: ts.Node): void {
    ...
    const targets = keys.get(key) ?? new Set<ts.Node>();
    keys.set(key, targets);
    if (targets.has(result)) return;
    targets.add(result);
    this.property(base, key, result);
  }
  private invoke(
    ...
    if (!this.reachable.has(target))
      this.activate(target, { root: path.root, chain: [...path.chain, this.locus(call)] });
    ...
    for (const receiver of this.receivers.get(target) ?? []) {
      if (ts.isNewExpression(call)) this.flow(call, receiver);
        ...
        this.flow(call.expression.expression, receiver);
    }
    for (const expression of this.returns.get(target) ?? []) this.flow(expression, call);
  }
  private flowInvocationArguments(
    ...
    for (let index = 0; index < target.parameters.length; index++) {
      ...
      if (parameter?.initializer) {
        const type = argument ? this.checker.getTypeAtLocation(argument) : undefined;
        if (
          !type ||
          (type.isUnion() ? type.types : [type]).some((part) =>
            Boolean(part.flags & ts.TypeFlags.Undefined),
          )
        )
          this.flow(parameter.initializer, parameter);
      }
    }
  }
  ...
  private recordInvocation(
    ...
    const callbacks = this.callbackTargets.get(call) ?? new Set<ts.Node>();
    callbacks.add(target);
    this.callbackTargets.set(call, callbacks);
    if (direct && ts.isCallExpression(call)) {
      const targets = this.targets.get(call) ?? new Set<ts.Node>();
      targets.add(target);
      this.targets.set(call, targets);
    }
    const sites =
      this.invocationSites.get(target) ??
      new Map<
        ts.CallExpression | ts.NewExpression,
        { arguments: readonly ts.Expression[]; synchronous: boolean }
      >();
    if (!sites.has(call)) {
      const owner = this.nativeOwner(call),
        name = memberName(call.expression);
      const synchronous =
        direct ||
        (ts.isCallExpression(call) && this.eventEmissions.has(call)) ||
        (/typescript\/lib\//.test(owner) && !["then", "catch", "finally"].includes(name)) ||
        /async_hooks\.d\.ts$/.test(owner);
      sites.set(call, { arguments: arguments_, synchronous });
      this.invocationSites.set(target, sites);
      this.queue.push(() => this.dispatchEvents());
    }
  }

  private externalEventOrigin(
    receiver: ts.Node,
    ...
    if (value && ts.isNewExpression(value) && /\/net\.d\.ts$/.test(this.nativeOwner(value))) {
      const origins = this.runtimeValues(
        receiver,
        new Map<ts.ParameterDeclaration, ts.Expression>(),
      );
      const connect = this.calls.find(
        (call) =>
          this.path(call) &&
          ts.isPropertyAccessExpression(call.expression) &&
          memberName(call.expression) === "connect" &&
          /\/net\.d\.ts$/.test(this.nativeOwner(call)) &&
          this.runtimeValues(
            call.expression.expression,
            new Map<ts.ParameterDeclaration, ts.Expression>(),
          ).some((origin) => origins.includes(origin)),
      );
    ...
    if (value && (ts.isCallExpression(value) || ts.isNewExpression(value)) && this.path(value)) {
      ...
      if (
        ...
        return value;
      if (/lib\.dom\.d\.ts$|bun-types\//.test(owner) && ["WebSocket", "EventSource"].includes(name))
        return value;
    }
    ...
    return this.memberEventOrigin(receiver, seen);
  }
  private memberEventOrigin(
    receiver: ts.Node,
    seen: Set<ts.Node>,
  ): ts.CallExpression | ts.NewExpression | undefined {
    if (
      (ts.isPropertyAccessExpression(receiver) || ts.isElementAccessExpression(receiver)) &&
      ["stdout", "stderr"].includes(memberName(receiver)) &&
      /\/child_process\.d\.ts$/.test(this.declaration(receiver)?.getSourceFile().fileName ?? "")
    ) {
      const origin = this.externalEventOrigin(receiver.expression, seen);
      if (origin && /\/child_process\.d\.ts$/.test(this.nativeOwner(origin))) return origin;
    }
    return this.rendererEventOrigin(receiver);
  }
  private rendererReceiver(receiver: ts.Node): ts.Node | undefined {
    if (ts.isPropertyAccessExpression(receiver) && receiver.name.text === "webContents")
      return receiver.expression;
    const declaration = this.declaration(receiver);
    if (
      declaration &&
      ts.isBindingElement(declaration) &&
      memberName(declaration.propertyName ?? declaration.name) === "webContents"
    ) {
  ...
  private rendererEventOrigin(receiver: ts.Node): ts.CallExpression | undefined {
    const symbol = this.checker.getTypeAtLocation(receiver).getSymbol();
    if (
      symbol?.name !== "WebContents" ||
  ...
  private eventContexts(node: ts.Node, seen = new Set<ts.Node>()): EventContext[] {
    ...
    let resumed = false;
    walk(owner, (part) => {
      if (ts.isAwaitExpression(part) && scope(part) === owner && part.getEnd() < node.getStart())
        resumed = true;
    });
    if (resumed) return [];
    seen.add(owner);
    if (ts.isSourceFile(owner)) {
      if (this.roots.has(resolve(owner.fileName)))
        return [
          { sites: [owner, node], bindings: new Map<ts.ParameterDeclaration, ts.Expression>() },
        ];
      return [...this.links].flatMap(([, links]) =>
        links
          .filter((link) => link.target === owner)
          .flatMap((link) =>
            this.eventContexts(link.site, new Set(seen)).map((context) => ({
              ...context,
              sites: [...context.sites, node],
            })),
          ),
      );
    }
    return [...(this.invocationSites.get(owner) ?? [])].flatMap(([call, invocation]) => {
      ...
      const contexts = invocation.synchronous
        ? this.eventContexts(call, new Set(seen))
        : [{ sites: [owner], bindings: new Map<ts.ParameterDeclaration, ts.Expression>() }];
      return contexts.map((context) => {
        const bindings = new Map(context.bindings);
        for (let index = 0; index < owner.parameters.length; index++) {
          const parameter = owner.parameters[index],
  ...
  private eventOrder(before: EventContext, after: EventContext): boolean {
    ...
    for (let index = 0; index < Math.min(before.sites.length, after.sites.length); index++) {
      const left = before.sites[index],
        right = after.sites[index];
  ...
  private dispatchEvents(): void {
    for (const [registration, receiver] of this.eventRegistrations)
      this.dispatchRegistration(registration, receiver);
  }
  private dispatchRegistration(registration: ts.CallExpression, receiver: ts.Expression): void {
    ...
    if (!callback) return;
    const names = registration.arguments[0]
      ? this.strings(registration.arguments[0]).map((row) => row.value)
      : [];
    const origins = this.runtimeValues(receiver, new Map<ts.ParameterDeclaration, ts.Expression>());
    for (const [emission, emittedReceiver] of this.eventEmissions)
      this.dispatchEmission(
        registration,
        receiver,
        callback,
        names,
  ...
  private dispatchEmission(
    ...
    emittedReceiver: ts.Expression,
  ): void {
    const ordered = this.eventContexts(registration).flatMap((before) =>
      this.eventContexts(emission)
        .filter((after) => {
          if (
            !this.eventOrder(before, after) ||
            !this.runtimeValues(emittedReceiver, after.bindings).some((value) =>
              this.runtimeValues(receiver, before.bindings).includes(value),
            )
          )
            return false;
          const registered = registration.arguments[0]
            ? this.strings(
    ...
    const removed = this.listenerRemovals.some((call) => {
      if (
        ...
        !ordered.every(({ before, after }) =>
          this.eventContexts(call).some(
            (context) => this.eventOrder(before, context) && this.eventOrder(context, after),
          ),
        )
      )
        return false;
      if (
        !ts.isPropertyAccessExpression(call.expression) ||
        !["off", "removeListener", "removeAllListeners", "removeEventListener"].includes(
          memberName(call.expression),
        ) ||
        !/@types\/node\/|lib\.dom\.d\.ts$/.test(this.nativeOwner(call))
      )
        return false;
      if (
        !this.runtimeValues(
          call.expression.expression,
          new Map<ts.ParameterDeclaration, ts.Expression>(),
        ).some((value) => origins.includes(value))
      )
        return false;
      ...
      return (
        memberName(call.expression) === "removeAllListeners" ||
        (!!handler &&
          this.runtimeValues(handler, new Map<ts.ParameterDeclaration, ts.Expression>()).some(
            (value) =>
              this.runtimeValues(
                callback,
                new Map<ts.ParameterDeclaration, ts.Expression>(),
    ...
    for (const { before, after } of ordered)
      for (const target of this.runtimeValues(callback, before.bindings)) {
        const root = after.sites[0],
          path = this.path(emission);
        if (
          root &&
          ts.isSourceFile(root) &&
          path &&
          implementation(target) &&
          !this.reachable.has(target)
        )
          this.activate(target, {
            root: path.root,
            chain: after.sites.slice(1).map((site) => this.locus(site)),
          });
  ...
  private dispatchExternal(
    ...
    const nativeNames = names.filter((name) => {
      ...
      return (
        !contexts.length ||
        !contexts.every((before) =>
          removals.some((call) =>
            this.eventContexts(call).some((after) => {
              if (!this.eventOrder(before, after)) return false;
              const event = call.arguments[0],
                handler = call.arguments[1];
              if (
                event &&
                !this.strings(event, false, new Set<ts.Node>(), after.bindings).some(
                  (row) => row.value === name,
                )
              )
                return false;
              return (
                memberName(call.expression) === "removeAllListeners" ||
                (!!handler &&
                  this.runtimeValues(handler, after.bindings).some((value) =>
                    this.runtimeValues(callback, before.bindings).includes(value),
                  ))
              );
            }),
          ),
    ...
    if (this.nativeReady && contract && nativeNames.length) {
      ...
      this.externalEvents.set(registration, {
        source,
        events: nativeNames,
        declaration,
        declarationSha256: digest(readFileSync(contract.declaration.getSourceFile().fileName)),
      ...
      for (const target of this.points.get(callback) ?? []) {
        if (!this.reachable.has(target))
          this.activate(target, {
            root: path.root,
            chain: [...path.chain, source, this.locus(registration)],
          });
        this.invoke(registration, target, [], false);
      }
    }
  ...
  invokedWithin(owner: ts.Node): ts.Node[] {
    return [...this.callbackTargets]
      .filter(([call]) => scope(call) === owner && this.path(call))
      .flatMap(([, targets]) => [...targets]);
  }
  private flowNode(node: ts.Node): void {
    const plain = unwrap(node);
    if (plain !== node) {
      this.flow(plain, node);
      return;
    }
    if (
      isFunction(node) ||
    ...
    const execute = () => {
      ...
      const invokePromiseExecutor = (): void => {
        if (
          ts.isNewExpression(node) &&
          /typescript\/lib\/lib\.es2015\.promise\.d\.ts$/.test(file)
        ) {
  ...
  private callbackOwner(node: ts.CallExpression | ts.NewExpression): string {
    ...
    if (!file && ts.isPropertyAccessExpression(node.expression)) {
      ...
      if (
        ...
        memberName(base.expression) === "split" &&
        /typescript\/lib\//.test(this.nativeOwner(base))
      )
        file = this.nativeOwner(base);
    }
    return file;
  }
  private flowMapEntries(
    file: string,
    name: string,
    arguments_: ts.NodeArray<ts.Expression> | never[],
    node: ts.NewExpression | ts.CallExpression,
    receiver: ts.LeftHandSideExpression | undefined,
  ) {
    ...
    const flowMapOperation = (): void => {
      if (
        !/typescript\/lib\/lib\..*\.d\.ts$/.test(file) ||
        !receiver ||
        !["get", "set", "add", "values", "entries"].includes(name)
      )
        return;
      if (name === "set" || name === "add") {
        ...
        if (value)
          for (const container of this.points.get(receiver) ?? [])
            if (ts.isNewExpression(container)) this.flow(value, container);
        for (const argument of arguments_) this.registeredCallbacks.add(argument);
  ...
  private flowNativeContainers(
    ...
    if (
      receiver &&
      /typescript\/lib\/lib\..*\.d\.ts$/.test(file) &&
      ["filter", "reverse", "slice", "sort", "next"].includes(name)
    )
      this.flow(receiver, node);
    ...
    const invokeTransactionMode = (): void => {
      if (
        receiver &&
        ["immediate", "deferred", "exclusive"].includes(name) &&
        /bun-types\/sqlite\.d\.ts$/.test(file)
      ) {
        for (const transaction of this.points.get(receiver) ?? []) {
          ...
          if (callback)
            for (const implementation of this.points.get(callback) ?? [])
              this.invoke(node, implementation, arguments_);
        }
      }
    };
  ...
  private recordNativeEvents(
    node: ts.NewExpression | ts.CallExpression,
    receiver: ts.LeftHandSideExpression | undefined,
    eventCallback: boolean,
    arguments_: never[] | ts.NodeArray<ts.Expression>,
    name: string,
    file: string,
  ) {
    ...
    if (
      ...
      receiver &&
      name === "emit" &&
      /@types\/node\/(?:events|process)\.d\.ts$/.test(file)
    ) {
      this.eventEmissions.set(node, receiver);
      this.dispatchEvents();
    }
    if (
      ts.isCallExpression(node) &&
      receiver &&
      name === "abort" &&
      /lib\.dom\.d\.ts$|bun-types\//.test(file)
    ) {
      this.eventEmissions.set(node, receiver);
      this.dispatchEvents();
    }
  ...
  private invokeSourceTargets(
    ...
    const invokeConstructor = (): void => {
      if (ts.isNewExpression(node)) {
        ...
        if (declaration && ts.isClassDeclaration(declaration)) {
          for (const member of declaration.members)
            if (ts.isConstructorDeclaration(member)) this.invoke(node, member, arguments_);
        }
      }
    };
    invokeConstructor();
    for (const target of this.points.get(node.expression) ?? []) {
      ...
      if (
        ts.isCallExpression(target) &&
        /bun-types\/sqlite\.d\.ts$/.test(this.nativeOwner(target)) &&
        memberName(target.expression) === "transaction"
      ) {
  ...
  private flowExpressions(node: ts.Node) {
    if (ts.isConditionalExpression(node)) {
      this.flow(node.whenTrue, node);
      this.flow(node.whenFalse, node);
    }
    const flowBinaryExpression = (): void => {
      if (ts.isBinaryExpression(node)) {
        if (
          ...
          this.flow(node.left, node);
          this.flow(node.right, node);
        }
    ...
    if (ts.isReturnStatement(node) && node.expression) {
      const owner = scope(node),
        expressions = this.returns.get(owner) ?? [];
      expressions.push(node.expression);
      this.returns.set(owner, expressions);
    }
    ...
    if (ts.isArrayLiteralExpression(node))
      for (const element of node.elements)
        if (ts.isSpreadElement(element)) this.flow(element.expression, node);
    const flowIteration = (): void => {
      if (ts.isForOfStatement(node)) {
        const declaration = ts.isVariableDeclarationList(node.initializer)
          ...
          : undefined;
        if (declaration)
          this.watch(node.expression, () => {
            for (const value of this.points.get(node.expression) ?? []) {
              if (ts.isArrayLiteralExpression(value))
                for (const element of value.elements) this.flow(element, declaration);
    ...
    if (ts.isPropertyAccessExpression(node) || ts.isElementAccessExpression(node)) {
      ...
      if (ts.isPropertyAccessExpression(node) && node.name.text === "value") {
        ...
        if (
          ...
          memberName(receiver.expression) === "next" &&
          /typescript\/lib\//.test(this.nativeOwner(receiver))
        )
          this.flow(node.expression, node);
      }
      const declaration = this.declaration(node);
      if (declaration && !ts.isMethodSignature(declaration) && !ts.isPropertySignature(declaration))
        this.flow(declaration, node);
      if (key === "signal")
        this.watch(node.expression, () => {
          for (const value of this.points.get(node.expression) ?? [])
            if (
              ts.isNewExpression(value) &&
              memberName(value.expression) === "AbortController" &&
              /lib\.dom\.d\.ts$|bun-types\//.test(this.nativeOwner(value))
            )
              this.point(node, value);
        });
    }
  }
  ...
  private flowBindings(node: ts.Node) {
    if (node.kind === ts.SyntaxKind.ThisKeyword) {
      ...
      const receivers = this.receivers.get(owner) ?? [];
      receivers.push(node);
      this.receivers.set(owner, receivers);
    }
    ...
    if (
      ...
      this.flow(node.initializer, node);
    ...
    if (ts.isBindingElement(node)) {
      ...
      if (ts.isObjectBindingPattern(node.parent))
        this.propertyOnce(owner, memberName(node.propertyName ?? node.name), node);
      else this.propertyOnce(owner, String(node.parent.elements.indexOf(node)), node);
    }
  }

  private flowCollectionConstructor(
    node: ts.NewExpression | ts.CallExpression,
    file: string,
    name: string,
    arguments_: never[] | ts.NodeArray<ts.Expression>,
    ...
    if (
      !(ts.isNewExpression(node) && /typescript\/lib\//.test(file) && ["Map", "Set"].includes(name))
    )
      return;
    const input = arguments_[0];
    if (input)
      for (const value of this.points.get(input) ?? []) {
        ...
        if (name === "Set") for (const element of value.elements) this.flow(element, node);
          ...
          const flowMapTuples = (): void => {
            ...
            for (const element of value.elements) {
              ...
              if (pair && ts.isArrayLiteralExpression(pair) && pair.elements[1])
                this.flow(pair.elements[1], node);
            }
            if (
              directPair &&
              !ts.isArrayLiteralExpression(this.resolve(value.elements[0] ?? value) ?? value)
            )
              this.flow(directPair, node);
          };
          flowMapTuples();
        }
      }
  ...
  private flowSchemaCallbacks(
    ...
    this.point(node, node);
    const callbacks = this.schemaCallbacks.get(node) ?? new Set<ts.Node>();
    this.schemaCallbacks.set(node, callbacks);
    for (const argument of arguments_) {
      const callback = this.resolve(argument);
      if (callback && isFunction(callback)) {
        callbacks.add(callback);
        this.registeredCallbacks.add(argument);
      }
    }
    if (receiver)
      for (const value of this.points.get(receiver) ?? []) {
        for (const callback of this.schemaCallbacks.get(value) ?? []) {
          callbacks.add(callback);
          if (["parse", "safeParse", "parseAsync", "safeParseAsync"].includes(name))
            this.invoke(node, callback, arguments_, false);
        }
      }
    if (["parse", "safeParse", "parseAsync", "safeParseAsync"].includes(name) && arguments_[0])
      this.flow(arguments_[0], node);
  }

  private flowAsyncStorage(
    receiver: ts.LeftHandSideExpression | undefined,
    file: string,
    name: string,
    arguments_: never[] | ts.NodeArray<ts.Expression>,
    ...
    if (receiver && /@types\/node\/async_hooks\.d\.ts$/.test(file)) {
      ...
      if (name === "run" || name === "enterWith") {
        ...
        if (value && storage) this.flow(value, storage);
        const callback = arguments_[1];
        if (callback)
          for (const target of this.points.get(callback) ?? [])
            this.invoke(node, target, arguments_.slice(2), false);
      }
  ...
  private domEventTarget(receiver: ts.Node, registrationOnly = false): boolean {
    ...
    if (registrationOnly && ["AbortSignal", "EventTarget"].includes(symbol.name)) return true;
    const rendererRoot = [...this.roots.values()].some(
      (root) => root.symbol === "electron-vite" || root.path.endsWith("electron.vite.config.ts"),
    );
    return rendererRoot && /^(?:Document|Window|EventTarget|Element|HTML\w*Element)$/.test(symbol.name);
  }
  ...
  private invokeServerProperties(
    ...
    for (const property of value.properties) {
      if (
        !property.name ||
        !["fetch", "error", "websocket", "socket"].includes(memberName(property.name))
      )
        continue;
      for (const callback of this.points.get(property) ?? [])
        this.invokeServerCallback(callback, node);
    }
  }
  private invokeServerCallback(
    callback: ts.Node,
    node: ts.NewExpression | ts.CallExpression,
  ): void {
  ...
  private invokeAiCallbacks(
    ...
    if (/\/ai\/dist\/index\.d\.ts$/.test(file) && name === "streamText") {
      ...
      if (metadata.version !== "6.0.141")
        fail("tool_version", manifest, "AI SDK callback contract requires 6.0.141");
      const contract = this.dependencyContracts.get("ai") ?? {
        version: "6.0.141",
        sha256: digest(readFileSync(manifest)),
        operations: [],
      };
      if (
        !contract.operations.some((row) => JSON.stringify(row) === JSON.stringify(this.locus(node)))
      )
        contract.operations.push(this.locus(node));
      this.dependencyContracts.set("ai", contract);
      const watched = this.nativeWatches.get(node) ?? new Set<ts.Node>();
      this.nativeWatches.set(node, watched);
      const observe = (value: ts.Node): void => {
        if (!watched.has(value)) {
          watched.add(value);
          this.watch(value, execute);
        }
      };
      ...
      const propertyCallbacks = (property: ts.ObjectLiteralElementLike, tools: boolean): void => {
        ...
        if (
          ["onError", "onFinish", "onChunk", "onStepFinish"].includes(key) ||
          (tools && key === "execute")
        ) {
          for (const target of this.points.get(property) ?? [])
            this.invoke(node, target, [], false);
        }
        if (key === "tools" || tools) {
      ...
      const callbacks = (value: ts.Node, tools = false): void => {
        ...
        for (const inner of this.points.get(value) ?? [])
          if (inner !== value) callbacks(inner, tools);
      };
      if (arguments_[0]) callbacks(arguments_[0]);
    }
  }

  private invokeNativeCallbacks(
    immediate: boolean,
    scheduled: boolean,
    arguments_: never[] | ts.NodeArray<ts.Expression>,
    node: ts.NewExpression | ts.CallExpression,
        ...
        const flowCallbackParameter = (): void => {
          if (immediate && isFunction(target) && receiver) {
            ...
            if (parameter)
              for (const value of this.points.get(receiver) ?? []) {
                if (ts.isArrayLiteralExpression(value))
                  for (const element of value.elements) this.flow(element, parameter);
  ...
  private invokeModuleFactory(
    ...
    if (ts.isCallExpression(node) && receiver && name === "module") {
      ...
      if (
        ...
        load.arguments[0] &&
        this.strings(load.arguments[0]).some((row) => row.value === "bun:test") &&
        ts.isCallExpression(load.expression) &&
        ts.isPropertyAccessExpression(load.expression.expression)
      ) {
        const provider = this.resolve(load.expression.expression.expression);
        if (
          provider &&
          /@types\/node\/module\.d\.ts$/.test(provider.getSourceFile().fileName) &&
          memberName(load.expression.expression) === "createRequire"
        ) {
          for (const argument of arguments_) this.registeredCallbacks.add(argument);
          const invokeImportedFactory = (): void => {
            const module = arguments_[0],
              factory = arguments_[1];
            if (
              module &&
              factory &&
              this.strings(module).some((row) =>
                this.sourceFiles.some((source) =>
                  source.statements.some(
                    (statement) =>
                      ts.isImportDeclaration(statement) &&
                      ts.isStringLiteral(statement.moduleSpecifier) &&
                      statement.moduleSpecifier.text === row.value,
                  ),
                ),
              )
            ) {
              for (const target of this.points.get(factory) ?? [])
                this.invoke(node, target, [], false);
            }
          };
  ...
  private activateSpawnedSource(
    ...
    if (
      ts.isCallExpression(node) &&
      /child_process\.d\.ts$|bun-types\//.test(file) &&
      ...
      if (input && ts.isObjectLiteralExpression(input)) {
        const property = input.properties.find(
          (property) => property.name && memberName(property.name) === "cmd",
        );
        if (property && ts.isPropertyAssignment(property))
          commands = this.commandArrays(property.initializer);
      }
      if (/child_process\.d\.ts$/.test(file) && arguments_[0] && arguments_[1]) {
        const binary = arguments_[0];
        commands = this.commandArrays(arguments_[1]).map((array) => [binary, ...array]);
      }
      const activateCommands = (): void => {
        for (const words of commands) {
          ...
          if (
            !binary ||
            !this.strings(binary).some(
              (row) =>
                row.value === process.execPath || /(?:^|\/)(?:bun|node|tsx)$/.test(row.value),
            )
          )
            continue;
          const entry = words[1];
          ...
          for (const value of this.strings(entry, true)) {
            const absolute = resolve(this.root, value.value),
              source =
                this.program.getSourceFile(absolute) ??
                this.program.getSourceFile(absolute.replace(/\.js$/, ".ts"));
  ...
  private flowComposedAbort(
    ...
    if (
      ts.isCallExpression(node) &&
      receiver &&
      memberName(receiver) === "AbortSignal" &&
      name === "any" &&
      /lib\.dom\.d\.ts$|bun-types\//.test(file)
    ) {
      ...
      if (signals)
        this.watch(signals, () => {
          for (const value of this.points.get(signals) ?? []) {
            if (ts.isArrayLiteralExpression(value))
              for (const signal of value.elements) this.flow(signal, node);
          }
        });
    }
  }

  private invokeElectronHandler(
    name: string,
    file: string,
    arguments_: never[] | ts.NodeArray<ts.Expression>,
  ) {
    ...
    for (const argument of arguments_) this.registeredCallbacks.add(argument);
    const channel = arguments_[0],
      callback = arguments_[1];
    if (channel && callback)
      for (const invocation of this.calls) {
        if (
          memberName(invocation.expression) !== "invoke" ||
          !/electron\/electron\.d\.ts$/.test(this.nativeOwner(invocation)) ||
          !this.path(invocation)
        )
          continue;
        const requested = invocation.arguments[0];
        if (
          !requested ||
          !this.strings(requested).some((entry) =>
            this.strings(channel).some((registered) => registered.value === entry.value),
          )
        )
          continue;
        for (const target of this.points.get(callback) ?? [])
          this.invoke(invocation, target, invocation.arguments.slice(1), false);
      }
  }

  structure(): {
    definition: Locus;
    implementations: Locus[];
    operations: Locus[];
    families: string[];
  }[] {
    const records: {
    ...
    for (const source of this.sourceFiles)
      walk(source, (node) => {
        if (
          ...
          node.parent.parent.name.text !== "Storage"
        )
          return;
        ...
        records.push({
          definition: this.locus(node, node.name.text),
          implementations,
          operations,
  ...
  private collectSqlFamilies(
    operation: ts.CallExpression,
    argument: ts.Expression | undefined,
    families: Set<string>,
  ): void {
    if (argument && /bun-types\/sqlite\.d\.ts$/.test(this.nativeOwner(operation)))
      for (const row of this.sql(argument)) {
        for (const match of row.value.matchAll(
          /\b(?:from|join|into|update)\s+["`[]?([\w]+)["`\]]?/gi,
        ))
          if (
            match[1] &&
            match[1].toLowerCase() !== "set" &&
            !match[1].includes("__CENSUS_DYNAMIC__")
          )
  ...
  role(node: ts.Node): "migration" | "archive" | "product" {
    ...
    for (const site of path?.chain ?? []) {
      for (const call of this.calls) {
        if (
          call.getSourceFile().fileName !== resolve(this.root, site.path) ||
          this.locus(call).line !== site.line
        )
          continue;
        for (const target of this.targets.get(call) ?? [])
  ...
  path(node: ts.Node): { root: Locus; chain: Locus[] } | undefined {
    return this.activeBranch(node) ? this.reachable.get(scope(node)) : undefined;
  }
  edge(origin: ts.Node, terminal: ts.Node, aliases: Locus[] = []): Edge | undefined {
    const path = this.path(terminal);
    if (!path) return undefined;
    return {
      originDefinition: this.locus(origin),
      importOrAliasPath: [
        ...this.importPath(terminal.getSourceFile(), origin.getSourceFile()),
        ...aliases,
      ],
      forwardingCallPath: path.chain,
      terminalProductOperation: this.locus(terminal),
      rootInvocation: path.root,
    };
  }
  private importPath(from: ts.SourceFile, to: ts.SourceFile): Locus[] {
    ...
    for (const [source, path] of paths) {
      ...
      for (const link of this.links.get(source) ?? []) {
        if (ts.isSourceFile(link.target) && !paths.has(link.target))
          paths.set(link.target, [...path, this.locus(link.site)]);
      }
    }
    return [];
  }
  strings(
    ...
    patterns = false,
    seen = new Set<ts.Node>(),
    bindings = new Map<ts.ParameterDeclaration, ts.Expression>(),
  ): { value: string; origin: ts.Node }[] {
    const node = unwrap(input);
    ...
    const environmentPathStrings = (): { value: string; origin: ts.Node }[] | undefined => {
      if (
        ...
        node.expression.getText() === "import.meta" &&
        ["dir", "dirname", "path", "filename", "url"].includes(node.name.text)
      ) {
        const path = relative(this.root, node.getSourceFile().fileName);
        return [
          {
            value: ["dir", "dirname"].includes(node.name.text) ? dirname(path) : path,
            origin: node,
          },
        ];
      }
      if (patterns && ts.isNewExpression(node) && memberName(node.expression) === "URL") {
        const part = node.arguments?.[0],
          base = node.arguments?.[1];
        if (part && base)
          return nested(base).flatMap((base) =>
            nested(part).map((part) => ({
              value: join(dirname(base.value), part.value),
              origin: node,
            })),
          );
      }
    ...
    const compoundStrings = (): { value: string; origin: ts.Node }[] | undefined => {
      ...
      if (ts.isBinaryExpression(node)) {
        if (node.operatorToken.kind === ts.SyntaxKind.PlusToken)
          return nested(node.left).flatMap((left) =>
            nested(node.right).map((right) => ({ value: left.value + right.value, origin: node })),
          );
        if (
          ...
          const left = nested(node.left),
            right = nested(node.right);
          return left.length && right.length ? [...left, ...right] : [];
        }
      }
      if (ts.isConditionalExpression(node)) {
        const yes = nested(node.whenTrue),
          no = nested(node.whenFalse);
        if (
          patterns &&
          node.whenFalse.kind === ts.SyntaxKind.Identifier &&
          node.whenFalse.getText() === "undefined"
        )
          return yes;
        if (
          patterns &&
          node.whenTrue.kind === ts.SyntaxKind.Identifier &&
          node.whenTrue.getText() === "undefined"
        )
          return no;
    ...
    const callStrings = (): { value: string; origin: ts.Node }[] | undefined => {
      if (ts.isCallExpression(node)) {
        const owner = this.nativeOwner(node),
          name = memberName(node.expression);
        const spawnOutputStrings = (): { value: string; origin: ts.Node }[] | undefined => {
          if (
            patterns &&
            /bun-types\//.test(owner) &&
            ["spawn", "spawnSync"].includes(name) &&
            node.arguments[0]
          ) {
            ...
            if (command && ts.isObjectLiteralExpression(command)) {
              ...
              if (property && ts.isPropertyAssignment(property))
                command = this.resolve(property.initializer);
            }
            if (command && ts.isArrayLiteralExpression(command)) {
              const words = command.elements.flatMap(nested).map((row) => row.value);
              if (words[0] === "git" && words.includes("ls-files"))
                return [{ value: "$inventory/**/*", origin: node }];
            }
          }
          return undefined;
        };
        ...
        const nativePathStrings = (): { value: string; origin: ts.Node }[] | undefined => {
          ...
          if (/\/node\/path\.d\.ts$/.test(owner) && ["join", "resolve"].includes(name)) {
            ...
            for (const argument of node.arguments) {
              const parts = nested(argument);
              if (!parts.length) return [];
              values = values.flatMap((value) =>
                parts.map((part) => (value ? join(value, part.value) : part.value)),
              );
            }
        ...
        const mappedJoinStrings = (): { value: string; origin: ts.Node }[] | undefined => {
          if (name === "join" && ts.isPropertyAccessExpression(node.expression)) {
            ...
            if (
              ...
              memberName(receiver.expression) === "map"
            ) {
              const mapper = receiver.arguments[0];
              if (mapper && ts.isArrowFunction(mapper) && ts.isStringLiteralLike(mapper.body))
                return [{ value: mapper.body.text, origin: mapper.body }];
            }
          }
          return undefined;
        };
        ...
        if (targets?.size)
          return [...targets].flatMap((target) => {
            ...
            return (this.returns.get(target) ?? []).flatMap((value) =>
              this.strings(value, patterns, new Set(seen), bound),
            );
          });
      }
      return undefined;
    ...
    const processStrings = (): { value: string; origin: ts.Node }[] | undefined => {
      if (
        ...
        return [{ value: `$env.${node.name.text}`, origin: node }];
      if (
        ts.isPropertyAccessExpression(node) &&
        node.name.text === "execPath" &&
        /@types\/node\/process\.d\.ts$/.test(this.declaration(node)?.getSourceFile().fileName ?? "")
      )
        return [{ value: process.execPath, origin: node }];
      if (
        patterns &&
        node.getText() === "process.argv" &&
        /@types\/node\/process\.d\.ts$/.test(this.declaration(node)?.getSourceFile().fileName ?? "")
      )
        return [{ value: "$argv", origin: node }];
      if (patterns && ts.isElementAccessExpression(node)) {
        const paths = nested(node.expression).filter((row) => row.value.startsWith("$argv"));
        if (paths.length)
          return paths.map((row) => ({
            value: `${row.value}.${node.argumentExpression.getText()}`,
            origin: node,
          }));
      }
      return undefined;
  ...
  private ambientCallStrings(
    node: ts.CallExpression,
    owner: string,
    name: string,
    nested: (node: ts.Node) => { value: string; origin: ts.Node }[],
  ): { value: string; origin: ts.Node }[] | undefined {
    if (/\/node\/url\.d\.ts$/.test(owner) && name === "fileURLToPath" && node.arguments[0])
      return nested(node.arguments[0]);
    if (/\/node_modules\//.test(owner) && name === "cwd") return [{ value: ".", origin: node }];
    if (/\/node\/fs\.d\.ts$/.test(owner) && name === "mkdtempSync" && node.arguments[0])
      return nested(node.arguments[0]).map((row) => ({ ...row, value: `${row.value}*` }));
    ...
    if (
      ...
      memberName(node.expression.expression.expression) === "randomBytes" &&
      /\/node\/crypto\.d\.ts$/.test(this.nativeOwner(node.expression.expression))
    )
      return [{ value: "*", origin: node }];
    return undefined;
  }
  ...
  private templateStrings(
    ...
    for (const span of node.templateSpans) {
      ...
      if (!parts.length) return [];
      values = values.flatMap((value) =>
        parts.map((part) => value + part.value + span.literal.text),
      );
    }
    return values.map((value) => ({ value, origin: node }));
  }

  private promiseBindingStrings(
    declaration: ts.BindingElement,
    nested: (node: ts.Node) => { value: string; origin: ts.Node }[],
  ): { value: string; origin: ts.Node }[] | undefined {
    const owner = declaration.parent.parent;
    if (!ts.isVariableDeclaration(owner) || !owner.initializer) return undefined;
    const initializer = ts.isAwaitExpression(owner.initializer)
      ? owner.initializer.expression
      : owner.initializer;
    if (
      !ts.isCallExpression(initializer) ||
      memberName(initializer.expression) !== "all" ||
      !initializer.arguments[0] ||
      !ts.isArrayLiteralExpression(initializer.arguments[0])
    )
      return undefined;
    const value =
      initializer.arguments[0].elements[declaration.parent.elements.indexOf(declaration)];
  ...
  private globCreation(value: ts.Node | undefined): ts.NewExpression | undefined {
    ...
    if (
      ...
      value.initializer &&
      ts.isNewExpression(value.initializer)
    )
  ...
  private globStrings(
    ...
    if (
      !ts.isCallExpression(node) ||
      !/bun-types\//.test(this.nativeOwner(node)) ||
      !["scan", "scanSync"].includes(memberName(node.expression)) ||
      !ts.isPropertyAccessExpression(node.expression)
    )
      return undefined;
  ...
  private iterableDeclarationStrings(
    ...
    if (
      !ts.isCallExpression(iterable) ||
      !ts.isPropertyAccessExpression(iterable.expression) ||
      !["scan", "scanSync"].includes(memberName(iterable.expression))
    )
      return undefined;
    const glob = this.declaration(iterable.expression.expression);
    if (
      ...
      !glob.initializer ||
      !ts.isNewExpression(glob.initializer) ||
      !glob.initializer.arguments?.[0]
    )
      return undefined;
    return nested(glob.initializer.arguments[0]);
  }
  private containerCallStrings(
    node: ts.Node,
    nested: (node: ts.Node) => { value: string; origin: ts.Node }[],
  ): { value: string; origin: ts.Node }[] | undefined {
    ...
    if (name === "slice" && /typescript\/lib\//.test(this.nativeOwner(node))) {
      const paths = nested(node.expression.expression).filter((row) =>
        row.value.startsWith("$argv"),
      );
      if (paths.length)
        return paths.map((row) => ({
          value: `${row.value}.slice(${node.arguments.map((argument) => argument.getText()).join(",")})`,
          origin: node,
        }));
    }
    if (["sort", "filter", "split", "toString", "trim", "catch"].includes(name))
      return nested(node.expression.expression);
    if (
      name === "text" &&
      /(?:bun-types\/|typescript\/lib\/lib\.dom\.d\.ts$)/.test(this.nativeOwner(node))
    ) {
      const receiver = this.resolve(node.expression.expression);
      if (
        receiver &&
        ts.isNewExpression(receiver) &&
        memberName(receiver.expression) === "Response" &&
        receiver.arguments?.[0]
      )
        return nested(receiver.arguments[0]);
    }
    if (name === "from" && node.arguments[0] && /typescript\/lib\//.test(this.nativeOwner(node)))
      return nested(node.arguments[0]);
    return undefined;
  }

  private resolvedStrings(
    node: ts.Node,
    patterns: boolean,
    bindings: Map<ts.ParameterDeclaration, ts.Expression>,
    nested: (node: ts.Node) => { value: string; origin: ts.Node }[],
  ): { value: string; origin: ts.Node }[] {
    const declaration = this.declaration(node);
    if (
    ...
    if (patterns && ts.isPropertyAccessExpression(node) && node.name.text === "stdout")
      return nested(node.expression);
    const patternBindingStrings = (): { value: string; origin: ts.Node }[] | undefined => {
      ...
      if (declaration && ts.isPropertyAssignment(declaration))
        return nested(declaration.initializer);
    ...
    const pointed = [...(this.points.get(node) ?? [])].filter(
      (value) => value !== node && (ts.isStringLiteralLike(value) || ts.isCallExpression(value)),
    );
    if (pointed.length) {
    ...
    const iterablePatternStrings = (): { value: string; origin: ts.Node }[] | undefined => {
      ...
      if (declaration && ts.isPropertySignature(declaration))
        return [{ value: `$input.${declaration.name.getText()}`, origin: declaration }];
  ...
  sql(node: ts.Node): { value: string; origin: ts.Node }[] {
    ...
    for (const span of resolved.templateSpans) {
      ...
      values = values.flatMap((value) =>
        (parts.length ? parts : [{ value: "__CENSUS_DYNAMIC__" }]).map(
          (part) => value + part.value + span.literal.text,
        ),
      );
    }
  ...
  commandArrays(node: ts.Node, seen = new Set<ts.Node>()): ts.Expression[][] {
    ...
    if (ts.isConditionalExpression(value))
      return [...nested(value.whenTrue), ...nested(value.whenFalse)];
    const declaration = this.declaration(node);
    ...
    if (
      ...
      const owner = declaration.parent.parent;
      if (ts.isVariableDeclaration(owner) && owner.initializer)
        return nested(owner.initializer).map((array) =>
          array.slice(declaration.parent.elements.indexOf(declaration)),
        );
    }
    if (declaration && ts.isParameter(declaration) && isFunction(declaration.parent)) {
      const owner = declaration.parent,
        index = owner.parameters.indexOf(declaration);
      return this.calls.flatMap((call) =>
        this.path(call) && this.targets.get(call)?.has(owner) && call.arguments[index]
          ? nested(call.arguments[index])
          : [],
      );
    }
    return [...(this.points.get(node) ?? [])]
      .filter((value) => value !== node && ts.isArrayLiteralExpression(value))
      .flatMap(nested);
  }
  runtimeValues(
    input: ts.Node,
    ...
    if (implementation(node) || ts.isObjectLiteralExpression(node) || ts.isNewExpression(node))
      return [node];
    const declaration = this.declaration(node);
    if (declaration && ts.isParameter(declaration) && bindings.has(declaration)) {
      const argument = bindings.get(declaration);
      return argument ? nested(argument) : [];
    }
    if (
      declaration &&
    ...
    if (ts.isPropertyAccessExpression(node)) {
      const properties = nested(node.expression).flatMap((value) =>
        ts.isObjectLiteralExpression(value)
          ? value.properties
              .filter((property) => property.name && memberName(property.name) === node.name.text)
              .flatMap(nested)
          : [],
      );
      if (properties.length) return properties;
    }
    if (declaration && (implementation(declaration) || this.nativeEventContract(node)))
      return [declaration];
    return [...(this.points.get(node) ?? [])];
  }
  publisherValues(call: ts.CallExpression, target: ts.Node): { value: string; origin: ts.Node }[] {
    ...
    const callers = this.calls.filter(
      (candidate) => this.path(candidate) && this.targets.get(candidate)?.has(owner),
    );
    ...
    for (const caller of callers) {
      ...
      for (let index = 0; index < owner.parameters.length; index++) {
        const parameter = owner.parameters[index],
          argument = caller.arguments[index];
        if (parameter && argument) bindings.set(parameter, argument);
      }
  ...
  values(node: ts.Node, seen = new Set<ts.Node>()): { value: string; origin: ts.Node }[] {
    ...
    if (ts.isCallExpression(target) && this.defineCall(target)) {
      const name = target.arguments[0];
      return name ? this.values(name, seen).map((row) => ({ ...row, origin: target })) : [];
    }
    if (ts.isObjectLiteralExpression(target)) {
      const name = target.properties.find((property) => property.name?.getText() === "name");
      if (name && ts.isPropertyAssignment(name))
        return this.values(name.initializer, seen).map((row) => ({ ...row, origin: target }));
      if (name && ts.isShorthandPropertyAssignment(name)) {
        ...
        if (declaration && (ts.isParameter(declaration) || ts.isVariableDeclaration(declaration)))
          return this.values(declaration.name, seen).map((row) => ({ ...row, origin: target }));
      }
    }
    if (ts.isParameter(target)) {
      ...
      return this.calls.flatMap((call) => {
        if (
          !this.path(call) ||
          !(this.targets.get(call)?.has(owner) || this.callTarget(call) === owner)
        )
          return [];
        const argument = call.arguments[index];
        return argument ? this.values(argument, new Set(seen)) : [];
      });
    }
    return [];
  }
  ...
  carries(node: ts.Node, parameter: ts.ParameterDeclaration, seen = new Set<ts.Node>()): boolean {
    ...
    if (
      ...
      (ts.isVariableDeclaration(declaration) || ts.isPropertyAssignment(declaration)) &&
      declaration.initializer &&
      this.carries(declaration.initializer, parameter, seen)
    )
      return true;
    let found = false;
    ts.forEachChild(node, (child) => {
      if (this.carries(child, parameter, seen)) found = true;
    });
    return found;
  }
  ...
  descriptor(node: ts.ObjectLiteralExpression): boolean {
    ...
    return (
      type.getSymbol()?.name === "Descriptor" &&
      node.properties.some((property) => property.name?.getText() === "name") &&
      node.properties.some((property) => property.name?.getText() === "schema")
    );
  }
}
function publisherCensus(graph: Provenance) {
  ...
  for (const source of graph.sourceFiles)
    walk(source, (node) => {
      if (
        !(ts.isCallExpression(node) && graph.defineCall(node)) &&
        !(ts.isObjectLiteralExpression(node) && graph.descriptor(node))
      )
        return;
      ...
      if (!values.length) {
        graph.problem(node, "dynamic_event_declaration");
        return;
      }
  ...
  for (const call of graph.calls) {
    ...
    const recordPublisherTarget = (target: ts.Node | undefined): void => {
      const name =
        target && (isFunction(target) || ts.isMethodSignature(target)) && target.name
          ? target.name.getText()
          : memberName(call.expression);
      if (name !== "publish") return;
      if (!target || ts.isMethodSignature(target) || ts.isPropertySignature(target)) {
        graph.problem(call, "unbound_publisher_port");
        return;
      }
      const owners = new Set<ts.Node>([target]);
      for (const owner of owners)
        for (const invoked of graph.invokedWithin(owner)) owners.add(invoked);
      const inner = graph.calls.filter((candidate) => owners.has(scope(candidate)));
      if (
        inner.some(
          (candidate) =>
            scope(candidate) === target && memberName(candidate.expression) === "publish",
        )
      )
        return;
      ...
      const terminal = inner.find((operation) => {
        ...
        const receiver = ts.isPropertyAccessExpression(operation.expression)
          ? operation.expression.expression
          : undefined;
        const name = memberName(operation.expression),
          owner = graph.nativeOwner(operation);
        if (receiver && name === "push" && ts.isObjectLiteralExpression(target.parent)) {
          ...
          const exposesCollection = target.parent.properties.some((property) => {
            if (ts.isShorthandPropertyAssignment(property))
              return (
                graph.checker.getShorthandAssignmentValueSymbol(property)?.valueDeclaration ===
                collected
              );
            return (
              ts.isPropertyAssignment(property) &&
              graph.declaration(property.initializer) === collected
            );
          });
          if (exposesCollection) return false;
        }
        const boundary =
          (/\/console\.d\.ts$|\/lib\.dom\.d\.ts$/.test(owner) &&
            ["log", "info", "warn", "error"].includes(name)) ||
          (!!receiver &&
            graph.checker.isArrayType(graph.checker.getTypeAtLocation(receiver)) &&
            name === "push");
        return (
          boundary && operation.arguments.some((argument) => graph.carries(argument, parameter))
        );
      });
      if (!terminal) return;
      const argument = call.arguments[0];
      ...
      if (!values.length) {
        graph.problem(call, "dynamic_publisher");
        return;
      }
      for (const row of values) {
    ...
    for (const target of implementations?.size ? implementations : [graph.callTarget(call)])
      recordPublisherTarget(target);
  }
  const records = [...new Set([...declarations.keys(), ...publishers.keys()])]
    .sort()
    .map((name) => {
      ...
      return {
        ...
        definition: declaration
          ? graph.locus(declaration, name)
          : edges[0]?.terminalProductOperation,
        productionPublishers: edges,
        declared: !!declaration,
      };
    });
  const findings: Finding[] = [];
  for (const record of records)
    if ((!record.declared || !record.productionPublishers.length) && record.definition)
      findings.push({ ...record.definition, symbol: record.name, class: "publisher" });
...
function publicMap(graph: Provenance) {
  ...
  recordFunctionUses();
  for (const file of graph.sourceFiles)
    walk(file, (node) => {
      ...
      if (
        ...
        row.aliases.push(graph.locus(node));
        return;
      }
  ...
  function followTypeLinks(): void {
    for (let changed = true; changed; ) {
      ...
      for (const link of typeLinks) {
        ...
        row.consumers.push(
          ...source.consumers.map((edge) => ({
            ...edge,
            originDefinition: graph.locus(link.to),
            importOrAliasPath: [...edge.importOrAliasPath, graph.locus(link.site)],
          })),
        );
        uses.set(link.to, row);
        changed = true;
      }
    }
  }
  ...
  function recordTypeLink(node: ts.Node, declaration: ts.Declaration): boolean {
    ...
    if (
      ...
      if (ts.isMethodSignature(owner)) {
        ...
        if (enclosing && ts.isInterfaceDeclaration(enclosing))
          typeLinks.push({ from: enclosing, to: declaration, site: node });
  ...
  function recordFunctionUses(): void {
    for (const call of graph.calls) {
      ...
      for (const target of graph.targets.get(call) ?? []) {
        ...
        if (
          !edge ||
          (!edge.forwardingCallPath.some((site) => site.path !== graph.locus(target).path) &&
            call.getSourceFile() === target.getSourceFile())
        )
          continue;
        const row = uses.get(target) ?? { consumers: [], aliases: [] };
        row.consumers.push(edge);
        uses.set(target, row);
      }
    }
  }
  ...
  for (const source of graph.sourceFiles) {
    ...
    for (const exported of graph.checker.getExportsOfModule(module)) {
      const symbol =
        exported.flags & ts.SymbolFlags.Alias ? graph.checker.getAliasedSymbol(exported) : exported;
  ...
  try {
    ...
    for (const dir of dirs) {
      const owned = files.filter(
        (row) =>
          eligible(row) &&
          (!topology || qualitySource(row.path)) &&
          ["typescript", "javascript"].includes(row.language) &&
          (dir === "."
            ? !topology || row.path.startsWith("script/")
            : row.path.startsWith(`${dir}/`)),
      );
      workspaces[dir] = {
        entry: [...roots.keys()]
          .filter((path) => owned.some((row) => resolve(root, row.path) === path))
          .map((path) => relative(resolve(root, dir), path)),
    ...
    function entries(value: Json, path: string, kind: string, parent = ""): void {
      if (Array.isArray(value)) {
        for (const item of value) entries(item, path, kind, parent);
        return;
      }
      const row = object(value);
      if (row.name === undefined) {
        for (const [name, value] of Object.entries(row)) entries(value, path, kind, name);
        return;
      }
      const name = kind === "files" ? "<file>" : string(row.name),
    ...
    for (const value of array(report.issues)) {
      ...
      for (const [kind, value] of Object.entries(row))
        if (kind !== "file") entries(value, path, kind);
    }
...
function storeCensus(graph: Provenance, tables: { name: string; sql: string }[]) {
  const migrations = graph.files
    .filter((row) => row.language === "sql" && row.category === "migration")
    .map((row) => ({
      path: row.path,
      sql: readFileSync(sourcePath(graph.root, row.path), "utf8"),
    }));
  ...
  const records = tables
    ...
    .map((table) => ({
      family: table.name,
      kind: "sqlite",
      liveTablesOrPathPattern: [table.name],
      definition: {
        path: origins.find((row) => row.family === table.name)?.path ?? "sqlite_schema",
        line: origins.find((row) => row.family === table.name)?.line ?? 1,
        symbol: table.name,
      },
      schemaVersion: digest(table.sql),
      adapter: [...ledgerCensusOwner(table.name)],
      productionReads: [] as Edge[],
      productionWrites: [] as Edge[],
      forwardingPaths: [] as Locus[],
      archiveOnlyReferences: [] as Locus[],
      migrationOnlyReferences: [] as Locus[],
      retentionOwner: "unassigned",
  ...
  function storeReceiver(call: ts.CallExpression) {
    const receiver = ts.isPropertyAccessExpression(call.expression)
      ? graph.resolve(call.expression.expression)
      : undefined;
    const preparation =
      receiver &&
      ts.isCallExpression(receiver) &&
      ["query", "prepare"].includes(memberName(receiver.expression))
        ? receiver
        : undefined;
    const bunFile =
      receiver &&
      ts.isCallExpression(receiver) &&
      memberName(receiver.expression) === "file" &&
      /bun-types\//.test(graph.nativeOwner(receiver))
        ? receiver
        : undefined;
  ...
  function nativeWrite(call: ts.CallExpression, native: string, name: string): boolean {
    ...
    const bunWrite =
      /bun-types\//.test(native) &&
      name === "write" &&
      ((!!writeDeclaration && ts.isFunctionDeclaration(writeDeclaration)) ||
        (ts.isPropertyAccessExpression(call.expression) &&
          call.expression.expression.getText() === "Bun"));
    return bunWrite;
  }
  function storeOperation(call: ts.CallExpression) {
    ...
    const filesystem =
      /@types\/node\/fs(?:\/promises)?\.d\.ts$/.test(native) &&
      [
        "writeFile",
        "writeFileSync",
        "readFile",
    ...
    if (
      ...
      !bunWrite &&
      !bunFile &&
      !(sqlite && ["exec", "run", "all", "get", "values", "iterate"].includes(name))
    )
      return undefined;
    if (bunFile && !["text", "json", "arrayBuffer", "bytes", "stream"].includes(name))
      return undefined;
    const argument = preparation
      ? preparation.arguments[0]
      : bunFile
        ? bunFile.arguments[0]
        : call.arguments[0];
  ...
  function recordUnresolvedStore(
    ...
    if (graph.role(call) === "migration") {
      ...
      for (const migration of migrations)
        historicalReferences.push({
          ...
          operation: graph.locus(call),
          role: sqlite ? "migration-execution" : "migration-source-read",
        });
    ...
    if (sqlite) {
      const owner = scope(call);
      const database = ts.isPropertyAccessExpression(call.expression)
        ? graph.resolve(call.expression.expression)
        : undefined;
      if (
        isFunction(owner) &&
        ledgerCensusSchemaCompiler(graph.locus(owner, owner.name?.getText() ?? "")) &&
        database &&
        ts.isNewExpression(database) &&
        database.arguments?.[0] &&
        graph.strings(database.arguments[0]).some((row) => row.value === ":memory:")
      ) {
        historicalReferences.push({
          family: ":memory:",
          operation: graph.locus(call),
          role: "drizzle-schema-comparison",
        });
        return;
      }
    }
  ...
  function recordSqlWrite(table: (typeof records)[number], sql: string, edge: Edge): void {
    const assigned = [
      ...(sql.match(/\bSET\s+([\s\S]+?)(?:\bWHERE\b|$)/i)?.[1] ?? "").matchAll(
        /\b([a-z_]\w*)\s*=/gi,
      ),
    ].map((match) => match[1] ?? "");
    const predicate = sql.match(/\bWHERE\s+([\s\S]+)/i)?.[1] ?? "";
    const guarded = assigned.some((column) =>
      new RegExp(`\\b${column}\\s*(?:=|IS\\s)`, "i").test(predicate),
    );
    table.productionWrites.push({
      ...edge,
      operationKind: guarded
        ? "compare-and-swap"
        : /^\s*insert\b/i.test(sql) && !/\b(?:replace|conflict)\b/i.test(sql)
          ? "append"
  ...
  const recordStoreCall = (call: ts.CallExpression): void => {
    ...
    if (!values.length) {
      recordUnresolvedStore(call, sqlite, argument);
      return;
    }
    const recordStoreValue = (row: { value: string; origin: ts.Node }): void => {
      if (!sqlite) {
        let table = records.find(
          (record) => record.kind === "filesystem" && record.family === row.value,
        );
        if (!table) {
          table = {
            family: row.value,
            kind: "filesystem",
            liveTablesOrPathPattern: [row.value],
            definition: graph.locus(row.origin, row.value),
            schemaVersion: digest(row.value),
            adapter: [graph.locus(call).path],
            productionReads: [],
            productionWrites: [],
            forwardingPaths: [],
            archiveOnlyReferences: [],
            migrationOnlyReferences: [],
            retentionOwner: "unassigned",
            dispositionReceipt: null,
          };
          records.push(table);
        ...
        if (edge) {
          if (bunFile || name === "readFile" || name === "readFileSync")
            table.productionReads.push(edge);
      ...
      const recordSqlFamily = (match: RegExpMatchArray): void => {
        ...
        if (family.includes("__CENSUS_DYNAMIC__")) {
          graph.problem(call, "dynamic_store_boundary");
          return;
        }
        ...
        if (!table) {
          if (origins.some((row) => row.family === family && row.dropped))
            historicalReferences.push({
              family,
              operation: graph.locus(call),
              role: "historical-schema",
            });
        ...
        if (role === "archive") {
          table.archiveOnlyReferences.push(locus);
          return;
        }
        if (role === "migration" && table.family !== "_migrations") {
          table.migrationOnlyReferences.push(locus);
          return;
        }
        ...
        const edge = graph.edge(row.origin, call);
        if (!edge) return;
        if (/^(from|join)$/i.test(match[1] ?? "") && !/^\s*delete/i.test(sql))
          table.productionReads.push(edge);
        else {
  ...
  const findings: Finding[] = records
    .filter((row) => !row.productionReads.length && !row.productionWrites.length)
    .map((row) => ({ ...row.definition, class: "store" }));
  return { records, findings, historicalReferences };
}
function pythonCensus(
  graph: Provenance,
  embedded: Entry[],
  embeddedText: Map<string, string>,
  executable: string,
): Json {
  const invocations = new Map<string, { root: Locus; chain: Locus[] }>();
  for (const [path, root] of graph.roots)
    invocations.set(relative(graph.root, path), { root, chain: [] });
  function commandProperty(command: ts.ObjectLiteralExpression): ts.Node | undefined {
    const cmd = command.properties.find(
      (property) => property.name && memberName(property.name) === "cmd",
    );
    if (cmd && ts.isShorthandPropertyAssignment(cmd)) {
      ...
      return declaration && ts.isVariableDeclaration(declaration) && declaration.initializer
        ? (graph.resolve(declaration.initializer) ?? declaration.initializer)
        : undefined;
    }
  ...
  function knownNonPythonCommands(arrays: readonly (readonly ts.Expression[])[]): boolean {
    ...
    return arrays.every((array) => {
      ...
      return (
        names.length > 0 && names.every((row) => !/(?:^|\/)python(?:3(?:\.\d+)?)?$/.test(row.value))
      );
    });
  }
  function pythonCommand(call: ts.CallExpression, owner: string) {
    ...
    if (!/bun-types\//.test(owner)) {
      ...
      if (call.arguments[0])
        words = [call.arguments[0], ...(arrays.length === 1 ? (arrays[0] ?? []) : [])];
  ...
  function pythonCwd(call: ts.CallExpression, options: ts.Node | undefined): string | undefined {
    ...
    if (options && ts.isObjectLiteralExpression(options)) {
      const property = options.properties.find(
        (property) => property.name && memberName(property.name) === "cwd",
      );
      if (property && ts.isPropertyAssignment(property)) {
        const directories = graph.strings(property.initializer, true);
        if (directories.length !== 1 || !directories[0] || /[$*]/.test(directories[0].value)) {
          graph.problem(call, "dynamic_process_cwd");
          return;
        }
  ...
  function recordEmbeddedPython(
    code: ts.Expression | undefined,
    call: ts.CallExpression,
    path: { root: Locus; chain: Locus[] },
  ): boolean {
    ...
    if (
      ...
      for (const [identity, text] of embeddedText)
        if (
          text === value.template.rawText &&
          identity.startsWith(`${relative(graph.root, value.getSourceFile().fileName)}#`)
        ) {
          invocations.set(identity, { root: path.root, chain: [...path.chain, graph.locus(call)] });
          identified = true;
        }
    }
    return identified;
  }
  function recordPythonWords(
    call: ts.CallExpression,
    path: { root: Locus; chain: Locus[] },
    words: readonly ts.Expression[],
    cwd: string,
  ): boolean {
    let identified = false;
    for (let index = 1; index < words.length; index++) {
      ...
      for (const value of values) {
        ...
        if (
          !graph.files.some(
            (row) => row.path === identity && eligible(row) && row.language === "python",
          )
        ) {
          graph.problem(call, "unresolved_python_process_source", word);
          continue;
        }
        invocations.set(identity, { root: path.root, chain: [...path.chain, graph.locus(call)] });
        identified = true;
  ...
  const recordPythonInvocation = (call: ts.CallExpression): void => {
    const path = graph.path(call),
      owner = graph.nativeOwner(call),
      name = memberName(call.expression);
    if (
      !path ||
      !/child_process\.d\.ts$|bun-types\//.test(owner) ||
      !["spawn", "spawnSync", "execFile", "execFileSync"].includes(name)
    )
      return;
    ...
    if (!binaries.length) {
      graph.problem(call, "dynamic_process_command");
      return;
    }
  ...
  const sources = [...graph.files, ...embedded]
    .filter((row) => eligible(row) && row.language === "python")
    .map((row) => {
      const text =
        embeddedText.get(row.path) ?? readFileSync(sourcePath(graph.root, row.path), "utf8");
  ...
  const result = Bun.spawnSync([executable, "-I", script], {
    stdin: Buffer.from(JSON.stringify({ sources })),
    timeout: 30_000,
  });
  if (result.exitCode !== 0 && result.exitCode !== 2)
    fail("python_execution", script, result.stderr.toString());
  const report = object(decode(result.stdout.toString()));
  if (report.version !== 1 || report.python !== "3.12.12")
    fail("tool_version", executable, "Python 3.12.12 required");
  if (array(report.sources).length !== sources.length)
    fail("python_partial", script, "source coverage differs");
  const analyzed = array(report.sources).map((value) => {
    ...
    for (const value of array(row.errors)) {
      ...
      graph.errors.push({
        path,
        line: typeof error.line === "number" ? error.line : 1,
    ...
    return {
      ...row,
      rootInvocation: invocation ? { ...invocation.root } : null,
      forwardingCallPath: invocation?.chain.map((site) => ({ ...site })) ?? [],
    };
  });
...
function locationOrder(a: Locus, b: Locus): number {
  return bytewise(a.path, b.path) || a.line - b.line || bytewise(a.symbol, b.symbol);
}
function censusOptions(argv: string[]) {
  return parseArgs({
    ...
    strict: true,
    options: {
      python: { type: "string", default: process.env.D945_PYTHON ?? "python3" },
      json: { type: "boolean" },
      ...
      knip: { type: "string" },
      "knip-sha256": { type: "string" },
      schema: { type: "string" },
      "schema-sha256": { type: "string" },
      "upgraded-schema": { type: "string" },
      "upgraded-schema-sha256": { type: "string" },
    },
  }).values;
}
function censusInput(values: ReturnType<typeof censusOptions>): {
  root: string;
  selected: CensusClass;
  input: ReturnType<typeof loadInput>;
} {
  const root = realpathSync(values.root);
  const selected = values.class;
  if (selected !== "publisher" && selected !== "export" && selected !== "store")
    fail("missing_input", "class", "select publisher, export or store");
  if (!values.inventory || !values["inventory-sha256"] || !values.contract)
    fail("missing_input", "inventory", "frozen inventory, digest and contract required");
...
export function censusMain(argv = Bun.argv.slice(2), shared = new CensusPrograms()): number {
  ...
  try {
    ...
    if (selected === "publisher") {
      const result = publisherCensus(graph);
    ...
    const document = {
      ...
      tools: {
        ...
        knip: selected === "export" ? "6.31.0" : null,
        knipHash: values["knip-sha256"] ?? null,
      },
      roots: [...roots.entries].map(([path, invocation]) => ({
        path: relative(root, path),
        invocation,
      })),
      configurationHashes: roots.configurationHashes,
      counts: { publisher: 0, export: 0, store: 0, [selected]: findings.length },
      analyzedClasses: [selected],
      findings,
      errors: graph.errors,
    ...
    if (jsonMode) console.log(JSON.stringify(document));
      ...
      for (const finding of findings)
        console.log(`${finding.class} ${finding.path}:${finding.line} ${finding.symbol}`);
    ...
    const failure = lastFailure ?? {
      code: "analysis_error",
      path: "",
      message: "input or analyzer failed",
    };
  ...
  function validateApplications(
    root: string,
    roots: ReturnType<typeof productionRoots>,
    graph: Provenance,
  ): void {
    for (const workspace of knipWorkspaces()) {
      if (
        workspace.dir.startsWith("apps/") &&
        ![...roots.entries.keys()].some((path) =>
          relative(root, path).startsWith(`${workspace.dir}/`),
        )
      )
        graph.errors.push({
          path: workspace.dir,
          line: 1,
          symbol: workspace.packageName,
          code: "unsupported_application_root",
        });
    }
  }
  ...
  function storeResults(
    selected: CensusClass,
    values: ReturnType<typeof censusOptions>,
    root: string,
    graph: Provenance,
    input: {
      files: Entry[];
    ...
    if (selected === "store") {
      if (
        !values.schema ||
        !values["schema-sha256"] ||
        !values["upgraded-schema"] ||
        !values["upgraded-schema-sha256"]
      )
        fail(
          "missing_input",
          "schema",
          "fresh and upgraded lifecycle databases with digests required",
        );
      const fresh = liveSchema(root, values.schema, values["schema-sha256"]),
        upgraded = liveSchema(root, values["upgraded-schema"], values["upgraded-schema-sha256"]);
      if (!fresh.length || JSON.stringify(fresh) !== JSON.stringify(upgraded))
        fail("schema_drift", "sqlite_schema", "fresh/upgraded schemas differ or empty");
      ...
      function attachBoundary(
        ...
        boundary: { [key: string]: Json },
        path: string,
      ): void {
        const locus = {
          path,
          line: typeof boundary.line === "number" ? boundary.line : 1,
          symbol: string(boundary.symbol),
        };
        ...
        const families =
          kind === "filesystem"
            ...
            : [...sql.matchAll(/\b(?:from|join|into|update)\s+["`[]?([\w]+)["`\]]?/gi)]
                .map((match) => match[1] ?? "")
                .filter((name) => name.toLowerCase() !== "set");
        for (const family of families)
          attachFamily(family, kind, sql, locus, path, source, boundary);
      }
      function attachFamily(
        ...
        if (!store && kind === "sqlite") {
          graph.errors.push({ ...locus, code: "unclassified_python_sql_family" });
          return;
        }
        if (!store) {
          store = {
            family,
            kind,
            liveTablesOrPathPattern: [family],
            definition: locus,
            schemaVersion: digest(family),
            adapter: [path],
            productionReads: [],
            productionWrites: [],
            forwardingPaths: [],
        ...
        const forwarding = array(source.forwardingCallPath).map((value) => {
          ...
          return {
            ...
            line: typeof site.line === "number" ? site.line : 1,
            symbol: string(site.symbol),
          };
        });
        const edge = {
          originDefinition: locus,
          ...
          forwardingCallPath: [
            ...forwarding,
            ...array(boundary.forwardingLines).map((line) => ({
              path,
          ...
          rootInvocation: {
            ...
            line: typeof invocation.line === "number" ? invocation.line : 1,
            symbol: string(invocation.symbol),
          },
        };
        if (boundary.read === true || (kind === "sqlite" && /^\s*(select|with)/i.test(sql)))
          store.productionReads.push(edge);
        if (
          boundary.write === true ||
          (kind === "sqlite" && /^\s*(insert|update|delete|replace)/i.test(sql))
        )
          store.productionWrites.push(edge);
      }

      findings = result.records
        .filter((row) => !row.productionReads.length && !row.productionWrites.length)
        .map((row) => ({ ...row.definition, class: "store" }));
      map = {
        stores: result.records,
        structuralContracts: graph.structure(),
        intrinsicStores: fresh
          .filter((row) => /\bAUTOINCREMENT\b/i.test(row.sql))
          .map((row) => ({
            family: "sqlite_sequence",
            maintainedFor: row.name,
            schemaVersion: digest(row.sql),
            consumerCredit: false,
          })),
        historicalReferences: result.historicalReferences,
        python,
      };
    }
    return { findings, map };
  ...
  function exportResults(
    ...
    roots: { entries: Map<string, Locus>; configurationHashes: { path: string; sha256: string }[] },
    previousFindings: Finding[],
    graph: Provenance,
    previousMap: object,
    scope?: ReturnType<typeof qualityPlan>,
    ...
    let map = previousMap;
    if (selected === "export") {
      if (!values.knip || !values["knip-sha256"])
        fail("missing_input", "knip", "pinned executable and digest required");
      if (digest(readFileSync(resolve(values.knip))) !== values["knip-sha256"])
        fail("tamper", values.knip, "Knip executable hash differs");
      const result = knipCensus(
        root,
        input.files,
        roots.entries,
        resolve(values.knip),
        ...
        if (
          !findings.some(
            (row) =>
              ...
              (row.symbol === finding.symbol || row.symbol === "<file>"),
          )
        )
    ...
    for (const call of graph.calls) {
      ...
      for (const argument of call.arguments) {
        ...
        if (
          ...
          !graph.reachable.has(callback) &&
          !targets?.size &&
          !graph.registeredCallbacks.has(argument) &&
          !ts.isParameter(graph.resolve(call.expression) ?? call)
        )
          graph.problem(call, "unresolved_callback_edge");
      }
      if (
        (call.expression.kind === ts.SyntaxKind.ImportKeyword && !graph.moduleCalls.has(call)) ||
        memberName(call.expression) === "eval"
      )
        graph.problem(call, "dynamic_module_boundary");
      if (
        ts.isElementAccessExpression(call.expression) &&
        !ts.isStringLiteralLike(call.expression.argumentExpression) &&
        !graph.nativeOwner(call).includes("/node_modules/")
      ) {
        const selector = call.expression.argumentExpression;
        const type = graph.checker.getTypeAtLocation(selector);
        const finite = (type.isUnion() ? type.types : [type]).every(
