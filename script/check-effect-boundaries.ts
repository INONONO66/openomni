...
class Provenance {
  ...
  private dynamicImport(node: ts.CallExpression, source: ts.SourceFile): Origin | undefined {
    return node.expression.kind === ts.SyntaxKind.ImportKeyword
      ? this.fromSpecifier(node.arguments[0], source)
      : undefined;
  }
  private bindingElement(node: ts.BindingElement, seen: Set<ts.Node | string>): Origin | undefined {
    ...
    return ts.isIdentifier(key) || ts.isStringLiteral(key)
      ? this.member(this.expression(declaration.initializer, seen), key.text, seen)
      : undefined;
  }
  expression(node: ts.Node, seen = new Set<ts.Node | string>()): Origin | undefined {
    ...
    if (ts.isCallExpression(node)) return this.dynamicImport(node, node.getSourceFile());
...
function analyze(worktree: string, files: readonly string[]): Analysis {
  ...
  if (diagnostics.length) return { sites: [], findings: diagnostics.map((diagnostic: ts.Diagnostic) => finding("ANALYSIS_ERROR", diagnostic.file ? relative(worktree, diagnostic.file.fileName) : "script", diagnostic.file ? diagnostic.file.getLineAndCharacterOfPosition(diagnostic.start ?? 0).line + 1 : 1)) };
...
export function checkEffectBoundaryFindings(worktree = root): readonly BoundaryFinding[] {
  ...
  const distinct = new Map(findings.map((entry: BoundaryFinding) => [`${entry.code}:${entry.file}:${entry.line}`, entry]));
