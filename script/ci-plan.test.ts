...
const planSchema = z
  .object({
    ...
    matrix: z
      .object({
        include: z.array(
          z.object({ key: z.string(), dir: z.string(), coverage: z.boolean() }).strict(),
        ),
      })
      .strict(),
  })
...
test.each([
  ...
  "mystery/file.ts",
  "packages/ui-extra/src/index.ts",
...
test("includes test-only permitted dependencies transitively without a workspace list", () => {
  ...
  const topology: readonly WorkspaceTopology[] = [
    ...TOPOLOGY,
    {
      ...sample,
      key: "future",
      dir: "packages/future",
      packageName: "@openomni/future",
      allowedDeps: ["@openomni/ui"],
      srcAllowedDeps: [],
    },
    {
      ...sample,
      key: "downstream",
      dir: "apps/downstream",
      packageName: "@openomni/downstream",
      allowedDeps: ["@openomni/future"],
    },
    {
      ...sample,
      key: "wildcard",
      dir: "apps/wildcard",
      packageName: "@openomni/wildcard",
      allowedDeps: "any-except-self",
    },
  ];
  // When UI changes, then both direct and transitive consumers are selected.
  expect(keys(["packages/ui/README.md"], topology)).toEqual([
    "ui",
    "desktopApp",
    "future",
    "downstream",
...
function fixture() {
  ...
  for (const workspace of TOPOLOGY) {
    ...
    writeFileSync(
      join(root, workspace.dir, "package.json"),
...
test.each(["push", "workflow_dispatch", "schedule"])("forces full for the %s event", (event) => {
  ...
  expect(result.exitCode).toBe(0);
  expect(planSchema.parse(JSON.parse(result.stdout.toString()))).toMatchObject({
    full: true,
    verify: true,
