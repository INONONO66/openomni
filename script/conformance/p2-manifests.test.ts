...
function issues(manifest: MutableManifest, extra: ReadonlyMap<string, string> = new Map()) {
  return validateP2Manifest(manifest, new Map([...sources, ...extra]));
}
...
describe("P2-00 exhaustive checked manifest discrimination", () => {
  ...
  test("rejects schema DDL drift and receipt text false positives", () => {
    ...
    expect(
      families(fixture(), new Map([[baselinePath, driftedBaseline]])).has("final-schema-signature"),
    ).toBe(true);
    ...
    expect(families(manifest, commentsOnly).has("missing-receipt")).toBe(true);
  });
