...
function fixture(): string {
  ...
  for (const name of ["topology.ts", "check-deps.ts", "check-import-cycles.ts", "lint-side-effects.ts", "lint-tools.test.ts"]) {
  ...
  for (const workspace of TOPOLOGY) {
    put(root, `${workspace.dir}/package.json`, JSON.stringify({ name: workspace.packageName, main: "src/index.ts" }));
...
function run(root: string, args: string[], cwd = root) {
  const result = Bun.spawnSync([process.execPath, ...args], { cwd, stdout: "pipe", stderr: "pipe", timeout: 15_000 });
...
for (const specifier of ["./panel", "./panel/index.tsx"]) {
  test(`cycle CLI discovers directory edge ${specifier}`, () => {
    const root = fixture();
    put(root, "packages/ui/src/entry.ts", `import { panel } from "${specifier}"; export const entry = panel;`);
    put(root, "packages/ui/src/panel/index.tsx", 'import { entry } from "../entry"; export const panel = entry;');
...
test("cycle CLI preserves external, asset, lazy and type-only edge exclusions", () => {
  const root = fixture();
  put(root, "packages/ui/src/entry.ts", 'import type { Panel } from "./panel"; import "react"; import "./style.css"; import("./panel"); export const entry = 1;');
  put(root, "packages/ui/src/panel/index.tsx", 'import { entry } from "../entry"; export type Panel = typeof entry;');
...
for (const cwd of [".", "script"]) {
  for (const planted of [false, true]) {
    test(`deleted-symbol census discovers production from ${cwd}, planted=${planted}`, () => {
      const root = fixture();
      put(root, "apps/openomni/src/planted.tsx", planted ? "export const TranscriptStore = 1;" : "export const live = 1;");
      put(root, "packages/ui/src/ignored.spec.tsx", "export const TranscriptStore = 1;");
      const result = run(root, ["test", join(root, "script/lint-tools.test.ts"), "--test-name-pattern", "deleted production symbols stay absent"], join(root, cwd));
...
test("deleted-symbol census rejects an empty production root", () => {
  ...
  const result = run(root, ["test", "script/lint-tools.test.ts", "--test-name-pattern", "deleted production symbols stay absent"]);
  expect(result.code).toBe(1);
});
...
for (const projected of [false, true]) {
  test(`side-effect CLI associates the emitting binding, projected=${projected}`, () => {
    ...
    put(root, "packages/llm/src/processor/index.ts", projected ? `function emit() { ${bind} sink.onMessage(message); }` : `function unrelated() { ${bind} } function emit(sink) { sink.onMessage(message); }`);
