...
describe("967 archive and guarded disposal", () => {
	test("enumerates every target and protected table before disposal", () => {
		...
		for (const table of ["bus_event", "wait", "message", "part", "session", "action", "inbox", "alarm", "ledger_event", "ledger_head", "event_chain"]) {
...
describe("967 real CLI disposal matrix", () => {
	test.each(["cancelled", "expired", "resolved"] as const)("disposes only archived eligible %s rows and is resumable", (status) => {
		...
		const changedTables = ["bus_event", "wait", "_migrations", "sqlite_sequence", "reply_grant", ...emptyRetiredTables];
		expect(after.tables.filter(({ name }) => !changedTables.includes(name))).toEqual(before.tables.filter(({ name }) => !changedTables.includes(name)));
