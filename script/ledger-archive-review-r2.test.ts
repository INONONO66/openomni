...
function resultCode(result: ReturnType<typeof archiveCli>): string {
  const line = (result.exitCode === 0 ? result.stdout : result.stderr)
    .split("\n")
...
function refusal(fixture: Fixture, flags: readonly string[]) {
  ...
  const observed = {
    ...
    mode: after.mode,
    logicalUnchanged: isDeepStrictEqual(after.logical, before.logical),
    bytesUnchanged: after.hash === before.hash,
    markersUnchanged: isDeepStrictEqual(after.markers, before.markers),
  };
  console.log(
    JSON.stringify({ refusal: observed, beforeHash: before.hash, afterHash: after.hash }),
  );
  return observed;
}
...
const literalProbes = [
  [
    "partial without replies",
    "UPDATE wait SET partial=1, data=json_set(data,'$.partial',json('true')) WHERE id='retired'",
  ],
  [
    "duplicate actions",
    `UPDATE wait SET data=json_set(data,'$.allowedActions', json('["report_result","report_result"]')) WHERE id='retired'`,
  ],
  [
    "impossible reply time",
    `UPDATE wait SET data=json_set(data,'$.replies', json('[{"replyKey":"r","responderId":"alice","receivedAt":9999999999999}]')) WHERE id='retired'`,
  ],
] as const;
...
describe("967 review R2-1 literal CLI eligibility probes", () => {
  for (const owner of ["workItem", "session"] as const) {
    test.each(literalProbes)(`${owner} refuses %s without any source mutation`, (_name, sql) => {
      ...
      fixture.db.run(
        "UPDATE wait SET owner_kind=?, data=json_set(data,'$.ownerRef.kind',?) WHERE id='retired'",
        [owner, owner],
      );
      fixture.db.run(sql);
      ...
      expect(fixture.db.query("SELECT id FROM wait WHERE id='retired'").all()).toEqual([
        { id: "retired" },
      ]);
    });
  }
  ...
  test.each([
    ...
    "resolved",
    "open",
  ] as const)("rejects %s replies before creation or beyond the last update", (status) => {
    using fixture = createDispositionFixture();
  ...
  test.each([
    ...
    "expired",
    "resolved",
  ] as const)("rejects %s replies outside the terminal window even before updatedAt", (status) => {
    using fixture = createDispositionFixture();
    seedRetiredWait(fixture.db, status);
    ...
    fixture.db.run(
      `UPDATE wait SET time_updated=200, data=json_set(data,'$.updatedAt',200,'$.replies',json(?)) WHERE id='retired'`,
      [JSON.stringify([{ replyKey: "r", responderId: "alice", receivedAt: at }])],
    );
  ...
  for (const policy of ["first_reply", "quorum", "all"] as const) {
    ...
    for (const vector of vectors) {
      test.each([
        "workItem",
        "session",
      ] as const)(`${policy} refuses ${vector} resolution for %s`, (owner) => {
        using fixture = createDispositionFixture();
        ...
        const replies = [
          ...
          ...(policy === "first_reply"
            ? []
            : [{ replyKey: "bob", responderId: "bob", receivedAt: 3 }]),
          ...(vector === "same-responder-duplicate-early"
            ? [{ replyKey: "alice-earlier", responderId: "alice", receivedAt: 1.5 }]
            : []),
        ];
        fixture.db.run(
          `UPDATE wait SET owner_kind=?, revision=2, time_updated=3,
        ...
        if (policy === "quorum")
          fixture.db.run(
            "UPDATE wait SET data=json_set(data,'$.quorum',json(?)) WHERE id='retired'",
        ...
        const row = fixture.db
          .query<{ data: string }, []>("SELECT data FROM wait WHERE id='retired'")
          .get();
  ...
  test.each([
    "workItem",
    "session",
  ] as const)("preserves distinct reply keys from the same %s responder with fractional times", (owner) => {
    // Given: valid follow-up replies, including one after resolvedAt, not duplicate reply identities.
    using fixture = createDispositionFixture();
    seedRetiredWait(fixture.db, "resolved");
    fixture.db.run(
      `UPDATE wait SET owner_kind=?, revision=2, time_created=1.25, time_updated=2.75, follow_up_until=102.5,
      data=json_set(data,'$.ownerRef.kind',?,'$.revision',2,'$.createdAt',1.25,'$.updatedAt',2.75,'$.resolvedAt',2.5,
      '$.replies',json('[{"replyKey":"r1","responderId":"alice","receivedAt":2.5},{"replyKey":"r2","responderId":"alice","receivedAt":2.75}]')) WHERE id='retired'`,
      [owner, owner],
    );
    ...
    expect({ exit: disposed.exitCode, code: resultCode(disposed) }).toEqual({
      exit: 0,
      code: "disposed",
    });
    ...
    expect(after.tables.filter(({ name }) => !migrated.includes(name))).toEqual(
      before.tables.filter(({ name }) => !migrated.includes(name)),
    );
    expect(after.tables.find(({ name }) => name === "reply_grant")?.rows).toEqual([]);
    expect(
      after.tables.filter(({ name }) =>
        ["delegation", "worker_grant", "worker_run_state"].includes(name),
      ),
    ).toEqual([]);
    expect(after.tables.find(({ name }) => name === "wait")?.rows).toEqual(
      before.tables
        .find(({ name }) => name === "wait")
        ?.rows.filter((row) => owner === "session" || row.id !== "retired"),
    );
    expect(fileSha256(fixture.archive)).toBe(archiveHash);
    expect(readFileSync(fixture.manifest)).toEqual(receipt);
    expect(manifestHash(fixture)).toBe(hash);
...
describe("967 review R2-2 same-schema archives", () => {
  test.each([
    "fresh",
    "upgraded",
  ] as const)("verifies a newly archived %s 0034 database", (source) => {
    using fixture = createDispositionFixture();
    if (source === "fresh") {
    ...
    const current = {
      ...fixture,
      path,
      manifest: `${fixture.directory}/new-manifest.json`,
      archive: `${fixture.directory}/new-archive.sqlite`,
    };
    ...
    expect({ exit: verified.exitCode, code: resultCode(verified) }).toEqual({
      exit: 0,
      code: "verified",
    });
    expect(snapshotDatabase(db)).toEqual(before);
    const acknowledged = disposeCli(current);
    expect({ exit: acknowledged.exitCode, code: resultCode(acknowledged) }).toEqual({
      exit: 0,
      code: "already_applied",
    });
    expect(snapshotDatabase(db)).toEqual(before);
    expect(fileSha256(current.archive)).toBe(hash);
    expect(db.query("SELECT name FROM _migrations WHERE name=?").all(U967_MIGRATION)).toEqual([
      { name: U967_MIGRATION },
    ]);
    if (source === "upgraded") {
      const repeated = disposeCli(fixture);
      expect({ exit: repeated.exitCode, code: resultCode(repeated) }).toEqual({
        exit: 0,
        code: "already_applied",
      });
  ...
  test.each([
    "missing",
    "unknown",
  ] as const)("refuses matching but invalid %s migration histories", (fault) => {
    using fixture = createDispositionFixture();
    if (fault === "missing")
      fixture.db.run("DELETE FROM _migrations WHERE name='0012_wait/migration.sql'");
...
describe("967 review R2-3 refusal before writable pragmas", () => {
  test("rechecks the exact receipt under the runner lock before any DELETE", async () => {
    ...
    const child = Bun.spawn(
      [
        process.execPath,
        "--preload",
        resolve(import.meta.dir, "../packages/ledger/test/helpers/disposition-967-fault.ts"),
        resolve(import.meta.dir, "generate-ledger-archive-manifest.ts"),
        "--db",
        fixture.path,
        "--out",
        fixture.manifest,
  ...
  test.each([
    ...
    "source-identity",
    "ineligible",
  ] as const)("refuses %s with mode, main bytes, rows, schema and markers unchanged", (fault) => {
    using fixture = createDispositionFixture();
    seedRetiredWait(fixture.db);
    if (fault === "ineligible")
      fixture.db.run(
        "UPDATE wait SET status='open',revision=0,data=json_remove(json_set(data,'$.status','open','$.revision',0),'$.cancelledAt') WHERE id='retired'",
      );
    expect(archiveCli(fixture).exitCode).toBe(0);
