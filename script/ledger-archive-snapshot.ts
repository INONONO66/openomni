...
import { sqliteSchema, U967Error, U967_MIGRATION, RETIRED_TABLE_MIGRATION, REPLY_GRANT_MIGRATION } from "../packages/ledger/src/storage/u967-preflight";
...
export function assertArchiveEquality(source: Database, restored: Database, dispositionDelta = false): void {
	...
	if (dispositionDelta) {
		for (const table of ["delegation", "worker_grant", "worker_run_state"]) {
			if (sourceSchema.some((row) => row.name === table) || !archivedSchema.some((row) => row.name === table)) continue;
			using remaining = restored.prepare<{ present: number | bigint }, []>(`SELECT 1 AS present FROM ${sqlIdentifier(table)} LIMIT 1`);
			if (remaining.get() !== null) throw new U967Error(`stale_archive:${table}`);
			removedTables.add(table);
		}
		if (!archivedSchema.some((row) => row.name === "reply_grant")
	...
	if (!isDeepStrictEqual(sourceSchema.filter((row) => !addedTables.has(row.tbl_name)), expectedSchema)) throw new U967Error("stale_archive");
