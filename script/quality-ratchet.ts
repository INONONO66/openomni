...
function finding(value: Json) {
  ...
  const result = {
    ...
    ...(row.endLine === undefined ? {} : { endLine: jsonNumber(row.endLine) }),
    ...(row.count === undefined ? {} : { count: jsonNumber(row.count) }),
  };
  if (
    !result.path ||
    !result.symbol ||
    ...
    result.push(
      ...selected.filter((row) =>
        ranges.some(
          (range) =>
            range.count > 0 &&
            row.line <= range.start + range.count - 1 &&
            (row.endLine ?? row.line) >= range.start,
        ),
      ),
    );
  }
...
export function ratchetMain(argv = process.argv.slice(2)): number {
  try {
    ...
    const inventory = buildInventory(
      values.root,
      readContract(resolve(values.root, values.contract)),
    );
    ...
    const priorFiles = git(values.root, [
      "ls-tree",
      "--name-only",
      values.base,
      "--",
    ...
    const established = Boolean(priorFiles.trim());
    if (established) {
    ...
    // Once present in the Git base, touched source must have no findings.
    const touched = established ? touchedFindings(values.root, values.base, current.findings) : [];
